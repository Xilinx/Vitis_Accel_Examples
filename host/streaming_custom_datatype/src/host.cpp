...
int main(int argc, char** argv) {
    ...
    std::vector<RGBcolor, aligned_allocator<RGBcolor> > hwRgbImagestruct(image_size);
    std::vector<HSVcolor, aligned_allocator<HSVcolor> > hwHsvImagestruct(image_size);
    ...
    ext.flags = 0;
    OCL_CHECK(ret, rgb_stream = xcl::Stream::createStream(device.get(), XCL_STREAM_READ_ONLY, CL_STREAM, &ext, &ret));

    // Create write stream for argument 1 of kernel
    ext.flags = 1;
    OCL_CHECK(ret, hsv_stream = xcl::Stream::createStream(device.get(), XCL_STREAM_WRITE_ONLY, CL_STREAM, &ext, &ret));
    ...
    std::thread thr1{[&] {
        err = OCL_CHECK(ret, xcl::Stream::writeStream(rgb_stream, hwRgbImagestruct.data(), image_size_bytes_struct,
                                                      &b_wr_req, &ret))
    }};
    ...
    std::thread thr2{[&] {
        err = OCL_CHECK(
            ret, xcl::Stream::readStream(hsv_stream, hwHsvImagestruct.data(), image_size_bytes_struct, &b_rd_req, &ret))
    }};
    ...
    for (unsigned int i = 0; i < image_size; i++)
        hwHsvImage[i] =
            (ap_int<8>(hwHsvImagestruct[i].h), ap_int<8>(hwHsvImagestruct[i].s), ap_int<8>(hwHsvImagestruct[i].v));
