...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < NCU; i++) {
        ...
        ext.flags = 0;
        OCL_CHECK(ret, h2c_stream_a[i] =
                           xcl::Stream::createStream(device.get(), XCL_STREAM_READ_ONLY, CL_STREAM, &ext, &ret));
        ext.flags = 1;
        OCL_CHECK(ret, h2c_stream_b[i] =
                           xcl::Stream::createStream(device.get(), XCL_STREAM_READ_ONLY, CL_STREAM, &ext, &ret));

        // Create read stream for argument 2 of kernel
        ext.flags = 2;
        OCL_CHECK(
            ret, c2h_stream[i] = xcl::Stream::createStream(device.get(), XCL_STREAM_WRITE_ONLY, CL_STREAM, &ext, &ret));
    }
    ...
    for (int i = 0; i < NCU; i++) {
        ...
        OCL_CHECK(ret, xcl::Stream::writeStream(h2c_stream_a[i], (h_a.data() + i * no_of_elem), vector_size_bytes,
                                                &wr_req, &ret));
        ...
        OCL_CHECK(ret, xcl::Stream::writeStream(h2c_stream_b[i], (h_b.data() + i * no_of_elem), vector_size_bytes,
                                                &wr_req, &ret));
        ...
        OCL_CHECK(ret, xcl::Stream::readStream(c2h_stream[i], (hw_results.data() + i * no_of_elem), vector_size_bytes,
                                               &rd_req, &ret));
    }
