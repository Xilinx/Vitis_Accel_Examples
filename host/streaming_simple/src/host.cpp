...
int main(int argc, char** argv) {
    ...
    ext.flags = 1;
    OCL_CHECK(ret, h2c_stream_b = xcl::Stream::createStream(device.get(), XCL_STREAM_READ_ONLY, CL_STREAM, &ext, &ret));
    ...
    std::thread thr1(xcl::Stream::writeStream, h2c_stream_a, h_a.data(), vector_size_bytes, &b_wr_req, &ret);

    b_wr_req.priv_data = (void*)"b_write_b";
    // Thread 2 for writing data to input stream 2 independently in case of
    // default blocking transfers.
    std::thread thr2(xcl::Stream::writeStream, h2c_stream_b, h_b.data(), vector_size_bytes, &b_wr_req, &ret);
    ...
    // blocking transfers.
    std::thread thr3(xcl::Stream::readStream, c2h_stream, hw_results.data(), vector_size_bytes, &b_rd_req, &ret);
    ...
    // transfers.
    OCL_CHECK(ret, xcl::Stream::writeStream(h2c_stream_a, h_a.data(), vector_size_bytes, &nb_wr_req, &ret));

    nb_wr_req.priv_data = (void*)"nb_write_b";
    // Writing data to input stream 2 independently in case of non-blocking
    // transfers.
    OCL_CHECK(ret, xcl::Stream::writeStream(h2c_stream_b, h_b.data(), vector_size_bytes, &nb_wr_req, &ret));
    ...
    OCL_CHECK(ret, xcl::Stream::readStream(c2h_stream, hw_results.data(), vector_size_bytes, &nb_rd_req, &ret));
