...
int main(int argc, char** argv) {
    ...
    parser.addSwitch("--xclbin_file", "-x", "Name of the .xclbin file", "");
    ...
    if (xclbinFile.empty()) {
        std::cerr << "ERROR: xclbin file must be specified with the -x option" << std::endl;
        parser.printHelp();
        return EXIT_FAILURE;
    }
    if (access(xclbinFile.c_str(), R_OK) != 0) {
        std::cerr << "ERROR: " << xclbinFile.c_str() << " file not found" << std::endl;
        parser.printHelp();
        return EXIT_FAILURE;
    }
    ...
    std::cout << "\nTest parameters\n";
    std::cout << " - xclbin file   : " << xclbinFile.c_str() << std::endl;
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            for (int i = 0; i < 6; i++) {
                std::string krnl_name_full = "testKernel_" + std::to_string(i + 1);
    ...
    if (xcl::is_emulation() and !xcl::is_hw_emulation()) {
        std::cout
            << "\nNot reporting performance throughput for sw_emu as clock signal is not present for time calculation."
            << std::endl;
    }
