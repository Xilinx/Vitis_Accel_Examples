...
void writeBuffer_1(int512_t* mem, int64_t buf_size) {
...
void readBuffer_1(int512_t* mem, int64_t buf_size, int64_t& err) {
    int64_t tmp = 0;
...
void testKernelProc_1(int512_t* mem, int64_t buf_size, int direction, hls::stream<int64_t>& cmd) {
    if (direction == 0) {
        cmd.write(0); // Send a command to start the counter
        writeBuffer_1(mem, buf_size);
        cmd.write(0); // Send a command to stop the counter
    } else {
        int64_t err;
        cmd.write(0); // Send a command to start the counter
        readBuffer_1(mem, buf_size, err);
        cmd.write(err); // Send a command to stop the counter
    }
}

void perfCounterProc_1(hls::stream<int64_t>& cmd, int64_t* out, int direction) {
    int64_t val;
...
extern "C" {
void testKernel_1(int64_t buf_size, int direction, int64_t* perf, int512_t* mem) {
...
#pragma HLS DATAFLOW
    ...
    testKernelProc_1(mem, buf_size, direction, cmd);
    perfCounterProc_1(cmd, perf, direction);
}
}
