...
int main(int argc, char** argv) {
    ...
    ext1.flags = 0;
    OCL_CHECK(ret,
              h2c_stream_a = xcl::Stream::createStream(device.get(), XCL_STREAM_READ_ONLY, CL_STREAM, &ext1, &ret));
    ext1.flags = 1;
    OCL_CHECK(ret,
              h2c_stream_b = xcl::Stream::createStream(device.get(), XCL_STREAM_READ_ONLY, CL_STREAM, &ext1, &ret));
    ext2.flags = 0;
    OCL_CHECK(ret,
              h2c_stream_c = xcl::Stream::createStream(device.get(), XCL_STREAM_READ_ONLY, CL_STREAM, &ext2, &ret));

    // Create read stream for argument 2 of kernel
    cl_stream c2h_stream;
    ext2.flags = 2;
    OCL_CHECK(ret, c2h_stream = xcl::Stream::createStream(device.get(), XCL_STREAM_WRITE_ONLY, CL_STREAM, &ext2, &ret));
    ...
    // transfers.
    OCL_CHECK(ret, xcl::Stream::writeStream(h2c_stream_a, h_a.data(), vector_size_bytes, &wr_req, &ret));

    wr_req.priv_data = (void*)"write_b";
    // Writing data to input stream 2 independently in case of non-blocking
    // transfers.
    OCL_CHECK(ret, xcl::Stream::writeStream(h2c_stream_b, h_b.data(), vector_size_bytes, &wr_req, &ret));

    wr_req.priv_data = (void*)"write_c";
    // Writing data to input stream 2 independently in case of non-blocking
    // transfers.
    OCL_CHECK(ret, xcl::Stream::writeStream(h2c_stream_c, h_c.data(), vector_size_bytes, &wr_req, &ret));
    ...
    OCL_CHECK(ret, xcl::Stream::readStream(c2h_stream, hw_results.data(), vector_size_bytes, &rd_req, &ret));
