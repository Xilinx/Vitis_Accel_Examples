...
LINK_OUTPUT := $(BUILD_DIR)/krnl_mmult.link.xclbin
...
EXECUTABLE = ./p2p_fpga2fpga
EMCONFIG_DIR = $(TEMP_DIR)

############################## Setting Targets ##############################
.PHONY: all clean cleanall docs emconfig
all: check-platform check-device check-vitis $(EXECUTABLE) $(BUILD_DIR)/krnl_mmult.xclbin $(BUILD_DIR)/krnl_madd.xclbin emconfig
...
$(TEMP_DIR)/krnl_mmult.xo: src/krnl_mmult.cpp
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) -c -k krnl_mmult --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
$(TEMP_DIR)/krnl_madd.xo: src/krnl_madd.cpp
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) -c -k krnl_madd --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'

$(BUILD_DIR)/krnl_mmult.xclbin: $(TEMP_DIR)/krnl_mmult.xo
	mkdir -p $(BUILD_DIR)
	v++ $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(LINK_OUTPUT)' $(+)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.out_dir $(PACKAGE_OUT) -o $(BUILD_DIR)/krnl_mmult.xclbin
$(BUILD_DIR)/krnl_madd.xclbin: $(TEMP_DIR)/krnl_madd.xo
	mkdir -p $(BUILD_DIR)
	v++ $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(LINK_OUTPUT)' $(+)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.out_dir $(PACKAGE_OUT) -o $(BUILD_DIR)/krnl_madd.xclbin
