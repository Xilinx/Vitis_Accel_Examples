...
int main(int argc, char** argv) {
    ...
    for (unsigned int i = 0; i < num_Blocks; i++) {
        ...
        OCL_CHECK(ret, xcl::Stream::writeStream(h2c_Stream_a, (h_a.data() + i * no_of_elem), vector_size_bytes, &wr_req,
                                                &ret));
        ...
        OCL_CHECK(ret, xcl::Stream::readStream(c2h_Stream, (hw_results.data() + i * no_of_elem), vector_size_bytes,
                                               &rd_req, &ret));
    }

    // Checking the request completions
    std::cout << "\n clPollStreams for (" << num_compl << ") events (Blocks: " << num_Blocks
    ...
    auto duration = std::chrono::duration<double, std::nano>(total_end - total_start);
    double throput =
        ((double)size * sizeof(int) * 2) / (double)duration.count(); // Multiplied by 2 because read and write both done
