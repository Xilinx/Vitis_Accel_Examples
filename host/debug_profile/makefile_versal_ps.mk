...
VPP_FLAGS_krnl_vadd +=  --config ./vadd_krnl_vadd.cfg


EXECUTABLE = ./debug_profile
...
$(TEMP_DIR)/krnl_vadd.xo: src/vadd.cpp
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) $(VPP_FLAGS_krnl_vadd) -c -k krnl_vadd --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'

$(BUILD_DIR)/vadd.xclbin: $(TEMP_DIR)/krnl_vadd.xo
	mkdir -p $(BUILD_DIR)
	v++ $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(LINK_OUTPUT)' $(+)
ifeq ($(EMU_PS), X86)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.emu_ps x86 -o $(BUILD_DIR)/vadd.xclbin
endif
############################## Preparing sdcard ##############################
.PHONY: sd_card
