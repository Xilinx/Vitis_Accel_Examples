...
int DATA_NUM =
    (256 * 1024 * 1024) /
    sizeof(uint32_t); // 256MB = 256 x 1024 x 1024 / 4Bytes (uint32_t)
...
int main(int argc, char *argv[]) {
  ...
  OCL_CHECK(err, cl::Buffer buffer_in_b(context, CL_MEM_READ_ONLY |
                                                     CL_MEM_EXT_PTR_XILINX,
                                        size_in_bytes, &host_buffer_ext, &err));
  OCL_CHECK(err, cl::Buffer buffer_out(context, CL_MEM_WRITE_ONLY |
                                                    CL_MEM_EXT_PTR_XILINX,
                                       size_in_bytes, &host_buffer_ext, &err));
  ...
  OCL_CHECK(err, in_b = (uint32_t *)q.enqueueMapBuffer(
                     buffer_in_b, CL_TRUE, CL_MAP_WRITE, 0, size_in_bytes, NULL,
                     NULL, &err));
