...
LINK_OUTPUT := $(BUILD_DIR)/validate.link.xsa
...
VPP_PFLAGS := 
CMD_ARGS = -x $(BUILD_DIR)/validate.xclbin
SD_CARD := $(PACKAGE_OUT)
vck190_dfx_hw := false
...
EXECUTABLE = ./validate.exe
EMCONFIG_DIR = $(TEMP_DIR)

############################## Setting Targets ##############################
.PHONY: all clean cleanall docs emconfig
ifeq ($(EMU_PS), X86)
all: check-platform check-device $(EXECUTABLE) $(BUILD_DIR)/validate.xclbin emconfig
else
all: check-platform check-device check_edge_sw $(EXECUTABLE) $(BUILD_DIR)/validate.xclbin emconfig sd_card
endif
...
.PHONY: build
build: check-vitis check-device $(BUILD_DIR)/validate.xclbin
...
$(TEMP_DIR)/verify.xo: src/verify.cpp
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) -c -k verify --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
$(TEMP_DIR)/bandwidth.xo: src/bandwidth.cpp
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) -c -k bandwidth --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'

$(BUILD_DIR)/validate.xclbin: $(TEMP_DIR)/verify.xo $(TEMP_DIR)/bandwidth.xo
	mkdir -p $(BUILD_DIR)
	v++ $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(LINK_OUTPUT)' $(+)
ifeq ($(EMU_PS), X86)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.emu_ps x86 -o $(BUILD_DIR)/validate.xclbin
endif
############################## Preparing sdcard ##############################
.PHONY: sd_card
sd_card: gen_run_app $(SD_CARD)

$(SD_CARD): $(BUILD_DIR)/validate.xclbin $(EXECUTABLE)
ifeq ($(findstring vck190_base_dfx, $(PLATFORM)), vck190_base_dfx)
ifeq ($(TARGET),$(filter $(TARGET), hw))
	v++ $(VPP_FLAGS) -p $(LINK_OUTPUT) -o $(BUILD_DIR)/validate.xclbin 
	v++ $(VPP_PFLAGS) $(VPP_FLAGS) -p --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(BUILD_DIR)/validate.xclbin
vck190_dfx_hw := true
endif
endif
ifeq ($(vck190_dfx_hw), false)
	v++ $(VPP_PFLAGS) -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(EMCONFIG_DIR)/emconfig.json -o $(BUILD_DIR)/validate.xclbin
endif
