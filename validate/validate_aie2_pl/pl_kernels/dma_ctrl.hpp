...
static void setDmaBd(hls::stream<ap_axiu<32, 0, 0, 0>> &ctrlOut,
                     ap_uint<32> ctrlPktID, ap_uint<32> dma_bd_number,
...
static void enqueueDmaBd(hls::stream<ap_axiu<32, 0, 0, 0>> &ctrlOut,
                         ap_uint<32> ctrlPktID, ap_uint<32> dma_bd_number,
                         ap_uint<32> dma_channel, bool dma_s2mm) {
#pragma HLS inline

  ap_uint<32> reg_start_enqueue;
  if (dma_channel == 0) {
    reg_start_enqueue =
        dma_s2mm ? XAIEGBL_MEM_DMAS2MM0STAQUE : XAIEGBL_MEM_DMAMM2S0STAQUE;
  } else {
    reg_start_enqueue =
        dma_s2mm ? XAIEGBL_MEM_DMAS2MM1STAQUE : XAIEGBL_MEM_DMAMM2S1STAQUE;
  }
