...
static void controlPacketWrite(hls::stream<ap_axiu<32, 0, 0, 0>> &ctrlOut,
                               ap_uint<32> ctrlPktID, ap_uint<32> addr,
                               ap_uint<32> pldNum, ap_uint<32> pld0,
                               ap_uint<32> pld1 = 0, ap_uint<32> pld2 = 0,
                               ap_uint<32> pld3 = 0) {
#pragma HLS inline
  ...
  ap_uint<32> ctrlPktHeader =
      generateControlHeader(addr, beats, 0x0 /*write without response*/);
...
static void controlPacketBlockWrite(hls::stream<ap_axiu<32, 0, 0, 0>> &ctrlOut,
                                    ap_uint<32> ctrlPktID, ap_uint<32> addr,
                                    ap_uint<32> pldNum, ap_uint<32> *pldPtr) {
  for (unsigned int i = 0; i < pldNum; i += 4) {
    ...
    if (((i + 4) > pldNum))
      ...
      num_words = 4;
    controlPacketWrite(ctrlOut, ctrlPktID, addr + 4 * i, num_words, pldPtr[i],
                       pldPtr[i + 1], pldPtr[i + 2], pldPtr[i + 3]);
  }
}
