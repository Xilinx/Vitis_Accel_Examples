...
void plController::print_micro_codes() {
  int i = 0;
  bool last = false;
  while (!last) {
    ...
    case SET_AIE_ITERATION: {
      ...
      printf(
          "SET_AIE_ITERATION: num_iter=%d, iter_mem_addr=%d, ctrl_strm_id=%d\n",
          num_iter, iter_mem_addr, ctrl_strm_id);
      break;
    }
...
void plController::enqueue_set_aie_iteration(const std::string &graphName,
                                             int num_iter, int ctrl_nm) {
  if (num_iter < 0)
    throw std::runtime_error("Number of iteration < 0: " +
  ...
  printf("enqueue_set_aie_iteration(): INFO: cores in same row controlled by "
...
void plController::enqueue_enable_aie_cores(int ctrl_nm) {
  if (!set_num_iter)
...
void plController::enqueue_disable_aie_cores(int ctrl_nm) {
  for (int i = 0; i < ctrl_nm; ++i) {
    opcodeBuffer->add(DISABLE_AIE_CORES);
    opcodeBuffer->add(i);
  }
}
void plController::enqueue_sync() { opcodeBuffer->add(SYNC); }
void plController::enqueue_loop_begin(int trip_count) {
  opcodeBuffer->add(LOOP_BEGIN);
  opcodeBuffer->add(trip_count);
}
void plController::enqueue_loop_end() { opcodeBuffer->add(LOOP_END); }
void plController::enqueue_set_and_enqueue_dma_bd(const std::string &portName,
...
void plController::enqueue_update_aie_rtp(const std::string &rtpPort,
                                          int rtpVal, int id) {
  ...
  opcodeBuffer->add(id);
  printf("enqueue_graph_rtp_update(): INFO: ping_addr = %d, pong_addr = %d, "
...
void plController::enqueue_halt() { opcodeBuffer->add(HALT); }
void plController::enqueue_write(int addr, int val) {
...
std::vector<char> plController::read_xclbin(const std::string &fnm) {
  ...
  if (!stream)
    throw std::runtime_error("read_xclbin():Failed to open file '" + fnm +
                             "' for reading");
...
void plController::read_aie_metadata(const char *data, size_t size,
                                     ptree &aie_project) {
