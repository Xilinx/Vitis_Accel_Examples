...
LINK_OUTPUT := $(BUILD_DIR)/vconv.link.xclbin
...
ifeq ($(EMU_PS), X86)
all: check-platform check-device $(EXECUTABLE) $(BUILD_DIR)/vconv.xclbin emconfig
else
all: check-platform check-device check_edge_sw $(EXECUTABLE) $(BUILD_DIR)/vconv.xclbin emconfig sd_card
endif
...
.PHONY: build
build: check-vitis check-device $(BUILD_DIR)/vconv.xclbin
...
$(TEMP_DIR)/vconv.xo: src/vconv.cpp
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) -c -k vconv --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'

$(BUILD_DIR)/vconv.xclbin: $(TEMP_DIR)/vconv.xo
	mkdir -p $(BUILD_DIR)
	v++ $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(LINK_OUTPUT)' $(+)

ifeq ($(EMU_PS), X86)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.emu_ps x86 -o $(BUILD_DIR)/vconv.xclbin
endif
############################## Preparing sdcard ##############################
.PHONY: sd_card
...
$(SD_CARD): $(BUILD_DIR)/vconv.xclbin $(EXECUTABLE)
	v++ $(VPP_PFLAGS) -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(EMCONFIG_DIR)/emconfig.json -o $(BUILD_DIR)/vconv.xclbin
