...
LINK_OUTPUT := $(BUILD_DIR)/N_stage_Adders.link.xsa
...
VPP_PFLAGS := 
CMD_ARGS = $(BUILD_DIR)/N_stage_Adders.xclbin
SD_CARD := $(PACKAGE_OUT)
vck190_dfx_hw := false
...
VPP_FLAGS_N_stage_Adders +=  --config ./N_stage_Adders_N_stage_Adders.cfg


EXECUTABLE = ./dataflow_stream_array
EMCONFIG_DIR = $(TEMP_DIR)

############################## Setting Targets ##############################
.PHONY: all clean cleanall docs emconfig
ifeq ($(EMU_PS), X86)
all: check-platform check-device $(EXECUTABLE) $(BUILD_DIR)/N_stage_Adders.xclbin emconfig
else
all: check-platform check-device check_edge_sw $(EXECUTABLE) $(BUILD_DIR)/N_stage_Adders.xclbin emconfig sd_card
endif
...
.PHONY: build
build: check-vitis check-device $(BUILD_DIR)/N_stage_Adders.xclbin

.PHONY: xclbin
xclbin: build

############################## Setting Rules for Binary Containers (Building Kernels) ##############################
$(TEMP_DIR)/N_stage_Adders.xo: src/N_stage_adders.cpp
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) $(VPP_FLAGS_N_stage_Adders) -c -k N_stage_Adders --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'

$(BUILD_DIR)/N_stage_Adders.xclbin: $(TEMP_DIR)/N_stage_Adders.xo
	mkdir -p $(BUILD_DIR)
	v++ $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(LINK_OUTPUT)' $(+)
ifeq ($(EMU_PS), X86)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.emu_ps x86 -o $(BUILD_DIR)/N_stage_Adders.xclbin
endif
############################## Preparing sdcard ##############################
.PHONY: sd_card
sd_card: gen_run_app $(SD_CARD)

$(SD_CARD): $(BUILD_DIR)/N_stage_Adders.xclbin $(EXECUTABLE)
ifeq ($(findstring vck190_base_dfx, $(PLATFORM)), vck190_base_dfx)
ifeq ($(TARGET),$(filter $(TARGET), hw))
	v++ $(VPP_FLAGS) -p $(LINK_OUTPUT) -o $(BUILD_DIR)/N_stage_Adders.xclbin 
	v++ $(VPP_PFLAGS) $(VPP_FLAGS) -p --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(BUILD_DIR)/N_stage_Adders.xclbin
vck190_dfx_hw := true
endif
endif
ifeq ($(vck190_dfx_hw), false)
	v++ $(VPP_PFLAGS) -p $(LINK_OUTPUT) $(VPP_FLAGS) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(EMCONFIG_DIR)/emconfig.json -o $(BUILD_DIR)/N_stage_Adders.xclbin
endif
