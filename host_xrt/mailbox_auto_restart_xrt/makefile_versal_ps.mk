...
.PHONY: build
build: check-vitis check-device $(BUILD_DIR)/mbox_autorestart.xclbin
...
$(TEMP_DIR)/mbox_autorestart.xo: src/mbox_autorestart.cpp
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) $(VPP_FLAGS_mbox_autorestart) -c -k mbox_autorestart --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'

$(BUILD_DIR)/mbox_autorestart.xclbin: $(TEMP_DIR)/mbox_autorestart.xo
	mkdir -p $(BUILD_DIR)
	v++ $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(LINK_OUTPUT)' $(+)
ifeq ($(EMU_PS), X86)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) -o $(BUILD_DIR)/mbox_autorestart.xclbin
endif
...
$(SD_CARD): $(BUILD_DIR)/mbox_autorestart.xclbin $(EXECUTABLE)
ifeq ($(findstring vck190_base_dfx, $(PLATFORM)), vck190_base_dfx)
ifeq ($(TARGET),$(filter $(TARGET), hw))
	v++ $(VPP_FLAGS) -p $(LINK_OUTPUT) -o $(BUILD_DIR)/mbox_autorestart.xclbin 
	v++ $(VPP_PFLAGS) $(VPP_FLAGS) -p --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(BUILD_DIR)/mbox_autorestart.xclbin
vck190_dfx_hw := true
endif
endif
