...
VPP_PFLAGS := 
CMD_ARGS = $(BUILD_DIR)/krnl_mmult.xclbin
CXXFLAGS += -I$(XILINX_XRT)/include -I$(XILINX_VIVADO)/include -Wall -O0 -g -std=c++17
LDFLAGS += -L$(XILINX_XRT)/lib -pthread -lOpenCL -lxrt_coreutil -luuid

########################## Checking if PLATFORM in allowlist #######################
PLATFORM_BLOCKLIST += samsung vck zc u2_ nodma v70 
...
EXECUTABLE = ./kernel_chain
EMCONFIG_DIR = $(TEMP_DIR)

############################## Setting Targets ##############################
.PHONY: all clean cleanall docs emconfig
all: check-platform check-device check-vitis $(EXECUTABLE) $(BUILD_DIR)/krnl_mmult.xclbin emconfig
...
$(TEMP_DIR)/krnl_chain_mmult.xo: src/krnl_chain_mmult.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k krnl_chain_mmult --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
$(TEMP_DIR)/krnl_simple_mmult.xo: src/krnl_simple_mmult.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k krnl_simple_mmult --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
