...
class txin(x3ntg_top):
    def __init__(self):
        # Emulation Setup: Create sim_ipc connection to tx_in
        self.m_txInIpc = ipc_axis_multi_intf_util("tx_in")
    ...
    def kill_simulation(self):
        self.m_txInIpc.end_of_simulation()

    def sample_meta(self):
        return self.m_txInIpc.sample_payloads()
    ...
    def enqueue(self, network_frame, port, delay_val=5, bad_frame = x3ntg_errcode.GOOD_FRAME):
        ...
        tuserArray = bytearray(x3ntg_tuser_txwidth * stride)
        
        idx = 0
        for dix in range(0, stride):
            stride_idx = dix * x3ntg_tuser_txwidth
            if stride_idx == 0:
                # set data_sop
                ...
                # set sequence ID
                # Bits 24-27 contain msb of seqid
                tuserArray[stride_idx + 3] |= ((self.m_seqid % 256) >> 4)
                # Bits 20-23 contain lsb of seqid
                tuserArray[stride_idx + 2] |= ((self.m_seqid % 16) << 4)
                with self.sendLock:
                    self.m_seqid += 1

                tuserArray[stride_idx + 3] |= (port << 4)
                    
                # Packet length - 36 to 49 bits filled
                packet_length = dataLen
                dlenbytes = packet_length.to_bytes(2,'big')
                dlen_b1 = dlenbytes[0]
                tuserArray[stride_idx + 4] |= dlen_b1 << 4
                tuserArray[stride_idx + 5] |= dlen_b1 >> 4
                dlen_b2 = dlenbytes[1]
                tuserArray[stride_idx + 5] |= (dlen_b2 << 4) & 0xff
                tuserArray[stride_idx + 6] |= (dlen_b2 >> 4) & 0x03
            ...
            if dix == stride-1:
                ...
                # set bad packet
                # 53rd bit set to indicate bad packet
                if bad_frame == x3ntg_errcode.BAD_FRAME:
                    tuserArray[stride_idx + 6] |= 0x20
        ...
        with self.sendLock:
            ...
            self.m_txInIpc.add_barrier([portIdx], self.transmit_frames)

    def send(self):
        with self.sendLock:
            # Transport all the frames
            self.m_txInIpc.transport_payloads(self.transmit_frames)
