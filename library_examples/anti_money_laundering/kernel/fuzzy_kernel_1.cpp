...
#pragma HLS STREAM variable = ed_match_strm depth = 4
    ...
    for (int i = 0; i < ED_NUM; i++) {
#pragma HLS unroll
        editDistance<N, M, BIT>(dispatch_len1_strm[i], dispatch_str1_strm[i], dispatch_len2_strm[i],
                                dispatch_str2_strm[i], dispatch_med_strm[i], dispatch_e_strm[i], ed_e_strm[i],
                                ed_match_strm[i]);
    }
...
#pragma HLS unroll
    ...
    buf_out[0] = t;
#ifndef __SYNTHESIS__
// std::cout << "HW0 Match:" << t << std::endl;
#endif
}

extern "C" void fuzzy_kernel_1(int base_addr,
                               int entry_num,
                               ap_uint<32>* buf_in,
