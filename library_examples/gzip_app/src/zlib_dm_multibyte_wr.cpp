...
void zlib_dm_wr(uintMemWidth_t *in, uint32_t input_size,
...
#pragma HLS dataflow
  xf::compression::details::mm2sSimple<MULTIPLE_BYTES * 8, 64>(in, inHlsStream,
                                                               input_size);
  xf::compression::details::streamDownsizer<uint32_t, MULTIPLE_BYTES * 8, 16>(
...
#pragma HLS INTERFACE m_axi port = in offset = slave bundle =                  \
    gmem0 max_read_burst_length = 64 max_write_burst_length =                  \
        2 num_read_outstanding = 16 num_write_outstanding = 1
