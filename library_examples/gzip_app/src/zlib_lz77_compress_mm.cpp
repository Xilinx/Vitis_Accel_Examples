...
void lz77Core(hls::stream<ap_uint<8>> &inStream,
              ...
              hls::stream<uint32_t> &compressedSize, uint32_t input_size) {
...
#pragma HLS dataflow
  xf::compression::lzCompress<MATCH_LEN, MIN_MATCH, LZ_MAX_OFFSET_LIMIT>(
      inStream, compressdStream, input_size);
  xf::compression::lzBestMatchFilter<MATCH_LEN, 0>(
      compressdStream, compressdStream_1, input_size);
  xf::compression::lzBooster<MAX_MATCH_LEN>(compressdStream_1, boosterStream,
                                            input_size);
  xf::compression::lz77Divide(boosterStream, outStreamVec, outStreamVecEos,
                              outStreamTree, compressedSize, input_size);
}

void lz77(const uintMemWidth_t *in, uintMemWidth_t *out,
          const uint32_t input_idx[PARALLEL_BLOCK],
          const uint32_t output_idx[PARALLEL_BLOCK],
          const uint32_t input_size[PARALLEL_BLOCK],
          uint32_t output_size[PARALLEL_BLOCK], uint32_t *dyn_ltree_freq,
...
#pragma HLS dataflow
  // MM2S Call
  xf::compression::details::mm2multStreamSize<8, PARALLEL_BLOCK, GMEM_DWIDTH,
                                              GMEM_BURST_SIZE>(
...
#pragma HLS UNROLL
    ...
    lz77Core(inStream[i], outStreamVec[i], outStreamVecEos[i],
             outStreamTreeData[i], compressedSize[i], input_size[i]);
  }

  // S2MM Call
  xf::compression::details::multStream2MMFreq<uint32_t, 32, PARALLEL_BLOCK,
                                              GMEM_DWIDTH, GMEM_BURST_SIZE>(
      outStreamVec, outStreamVecEos, compressedSize, outStreamTreeData,
      output_idx, out, output_size, dyn_ltree_freq, dyn_dtree_freq);
}

extern "C" {
void xilLz77Compress(const uintMemWidth_t *in, uintMemWidth_t *out,
...
#pragma HLS ARRAY_PARTITION variable = output_block_size dim = 0 complete
  ...
  for (int i = 0; i < no_blocks; i += PARALLEL_BLOCK) {
    ...
    lz77(in, out, input_idx, output_idx, input_block_size, output_block_size,
         dyn_ltree_freq, dyn_dtree_freq);
