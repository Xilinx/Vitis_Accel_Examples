...
void streamDataK2dmSync(
    hls::stream<ap_uint<STREAMDWIDTH>> &out, hls::stream<bool> &bytEos,
    uint32_t readBlockSize, hls::stream<uint32_t> &dataSize,
    hls::stream<ap_axiu<STREAMDWIDTH, 0, 0, 0>> &dmOutStream,
    hls::stream<ap_axiu<64, 0, 0, 0>> &sizestreamk, uint32_t already_done,
...
#pragma HLS PIPELINE II = 1
    ...
    if (last) {
      ...
      if (outSize == 0) {
        outSize = readBlockSize + 1; // write 1 extra size if last block has the
                                     // size equal to readBlockSize
...
void zlib_dm_rd(uintMemWidth_t *out, uint32_t *encoded_size,
                uint32_t cur_status, uint32_t readBlockSize,
...
#pragma HLS dataflow
  streamDataK2dmSync<MULTIPLE_BYTES * 8>(
      outHlsStream, outHlsStream_eos, readBlockSize, outsize_val, outAxiStream,
      sizestreamk, cur_status, dcStatusStream);

  xf::compression::details::s2mmEosSimple<MULTIPLE_BYTES * 8, kGMemBurstSize>(
      out, outHlsStream, outHlsStream_eos, outsize_val, encoded_size);
}

extern "C" {
void xilZlibDmReader(
    uintMemWidth_t *out, uint32_t *encoded_size, uint32_t *status_flag,
    uint32_t read_block_size,
    hls::stream<ap_axiu<MULTIPLE_BYTES * 8, 0, 0, 0>> &outstreamk,
    hls::stream<ap_axiu<64, 0, 0, 0>> &sizestreamk) {
#pragma HLS INTERFACE m_axi port = out offset = slave bundle =                 \
    gmem0 max_read_burst_length = 2 max_write_burst_length = 128
...
#pragma HLS STREAM variable = dcStatusStream depth = 3
  ...
  zlib_dm_rd(out, encoded_size, st_flg, read_block_size, outstreamk,
             sizestreamk, dcStatusStream);
