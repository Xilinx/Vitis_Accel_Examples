...
void huffmanCore(hls::stream<ap_uint<32>> &inStreamVec,
                 ...
                 hls::stream<bool> &outStreamEos, uint32_t input_size,
                 uint32_t core_idx, hls::stream<uint16_t> &inStreamTree,
...
#pragma HLS dataflow
  ...
  xf::compression::huffmanEncoder(inStreamVec, bitVals, bitLen, input_size,
                                  inStreamTree, inStreamSize);

  xf::compression::details::bitPacking(bitVals, bitLen, outStreamVec,
                                       outStreamEos, compressSize);
}

void huffman(const xf::compression::uintMemWidth_t *in, uint32_t *lit_freq,
             ...
             uint32_t output_size[PARALLEL_BLOCK], uint32_t n_blocks) {
  const uint32_t c_gmemBSize = 32;
...
#pragma HLS dataflow
  for (uint8_t i = 0; i < n_blocks; i++) {
    xf::compression::zlibTreegenInMMOutStream(&(lit_freq[i * 1024]),
                                              &(dist_freq[i * 64]),
                                              codeStream[i], codeSize[i]);
  }
  xf::compression::details::mm2multStreamSize<32, PARALLEL_BLOCK, GMEM_DWIDTH,
                                              GMEM_BURST_SIZE>(
      in, input_idx, inStreamVec, input_size);

  for (uint8_t i = 0; i < PARALLEL_BLOCK; i++) {
#pragma HLS UNROLL
    huffmanCore(inStreamVec[i], outStreamVec[i], outStreamSize[i],
                outStreamEos[i], input_size[i], 0, codeStream[i], codeSize[i]);
  }

  xf::compression::details::multStream2MM<16, PARALLEL_BLOCK, GMEM_DWIDTH,
                                          GMEM_BURST_SIZE>(
...
#pragma HLS ARRAY_PARTITION variable = output_block_size dim = 0 complete
  ...
  for (int i = 0; i < no_blocks; i += PARALLEL_BLOCK) {
    ...
    huffman(in, &(dyn_ltree_freq[lit_idx]), &(dyn_dtree_freq[dist_idx]), out,
            input_idx, output_idx, input_block_size, output_block_size,
            n_blocks);
