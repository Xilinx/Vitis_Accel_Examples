...
bool CmdLineParser::addSwitch(const CmdSwitch &s) {
  ...
  if (cmd.key.find("--") != 0 || cmd.key.length() < 3) {
    LogError("The input key is invalid. Please start with -- and keep a length "
             ">= 3");
    return false;
  }
  ...
  if (cmd.shortcut.length() == 0) {
    ...
    cmd.shortcut = temp;
    LogInfo("Automatic shortcut assigned %s to %s", temp.c_str(),
            cmd.key.c_str());
  }
...
bool CmdLineParser::setDefaultKey(const char *key) {
  ...
  if (m_mapKeySwitch.find(strKey) != m_mapKeySwitch.end()) {
    ...
    if (pcmd != NULL) {
      if (pcmd->istoggle) {
        LogError(
            "Boolean command line options can not be used as default keys");
        return false;
      }
    }
...
void CmdLineParser::printHelp() {
  ...
  for (auto pcmd : m_vSwitches) {
    if (pcmd->key.size() > first_max)
      first_max = pcmd->key.size();
    if (pcmd->shortcut.size() > second_max)
      second_max = pcmd->shortcut.size();
    if (pcmd->desc.size() > third_max)
      third_max = pcmd->desc.size();
  }
