...
void xfZlib::_enqueue_reads(uint32_t bufSize, uint8_t* out, uint32_t* decompSize, int cu, uint32_t max_outbuf_size) {
    ...
    for (int i = 0; i < BUFCNT; i++) {
        OCL_CHECK(err, buffer_size[i] = new cl::Buffer(*m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
...
overlap:
    for (uint32_t brick = 0, itr = 0; brick < total_chunks; itr++, buffIdx = (itr % OVERLAP_BUF_COUNT)) {
        ...
        if (itr >= OVERLAP_BUF_COUNT) {
            ...
            for (uint32_t bIdx = 0; bIdx < blocksPerChunk[brick_flag_idx]; bIdx++) {
                ...
                std::memcpy(&out[outIdx], &h_buf_zlibout[0][buffIdx].data()[bIdx * block_size_in_bytes],
                            compressed_size);
        ...
        std::memcpy(h_buf_in[0][buffIdx].data(), &in[brick * host_buffer_size], sizeOfChunk[brick]);
        ...
        OCL_CHECK(
            err, err = m_q[buffIdx]->enqueueMigrateMemObjects({*(buffer_input[0][buffIdx])}, 0 /* 0 means from host*/));

        // Compress Fire Kernel invocation
        OCL_CHECK(err, err = m_q[buffIdx]->enqueueTask(*compress_kernel[0]));

        // Migrate memory - Map device to host buffers
        m_q[buffIdx]->enqueueMigrateMemObjects({*(buffer_zlib_output[0][buffIdx]), *(buffer_compress_size[0][buffIdx])},
    ...
    for (uint32_t ovr_itr = 0, brick = stride - overlap_buf_count; ovr_itr < leftover; ovr_itr++, brick++) {
        ...
        for (uint32_t bIdx = 0; bIdx < blocksPerChunk[brick_flag_idx]; bIdx++) {
            uint32_t compressed_size = h_compressSize[0][buffIdx].data()[bIdx];
            std::memcpy(&out[outIdx], &h_buf_zlibout[0][buffIdx].data()[bIdx * block_size_in_bytes], compressed_size);
            outIdx += compressed_size;
        }
    }
#else
