...
void zip(std::string &inFile_name, std::ofstream &outFile, uint8_t *zip_out,
         uint32_t enbytes) {
...
void gzip_headers(std::string &inFile_name, std::ofstream &outFile,
                  uint8_t *zip_out, uint32_t enbytes) {
...
void zlib_headers(std::string &inFile_name, std::ofstream &outFile,
                  uint8_t *zip_out, uint32_t enbytes) {
  outFile.put(120);
  outFile.put(1);
  outFile.write((char *)zip_out, enbytes);
...
uint64_t xfZlib::compress_file(std::string &inFile_name,
                               std::string &outFile_name, uint64_t input_size) {
  ...
  std::vector<uint8_t, zlib_aligned_allocator<uint8_t>> zlib_out(input_size *
                                                                 2);

  MEM_ALLOC_CHECK(zlib_in.resize(input_size), input_size, "Input Buffer");
  MEM_ALLOC_CHECK(zlib_out.resize(input_size * 2), input_size * 2,
                  "Output Buffer");
  ...
  // zlib Compress
  enbytes =
      compress(zlib_in.data(), zlib_out.data(), input_size, host_buffer_size);

  auto compress_API_end = std::chrono::high_resolution_clock::now();
  auto duration = std::chrono::duration<double, std::nano>(compress_API_end -
                                                           compress_API_start);
  compress_API_time_ns_1 += duration;

  float throughput_in_mbps_1 =
      (float)input_size * 1000 / compress_API_time_ns_1.count();
...
int xfZlib::init(const std::string &binaryFileName, uint8_t kidx) {
  ...
  if (num_platforms == 0) {
    std::cerr
        << "No Platforms were found this could be cased because of the OpenCL \
  ...
  OCL_CHECK(err,
            m_context = new cl::Context(m_device, props, NULL, NULL, &err));
  if (err) {
...
// auto fileBuf = xcl::read_binary_file(binaryFileName);
  ...
  if (access(binaryFileName.c_str(), R_OK) != 0) {
    std::cerr << "ERROR: " << binaryFileName.c_str()
              << " xclbin not available please build " << std::endl;
    m_err_code = 1;
    return 1;
  }
  ...
  ZOCL_CHECK_2(err, m_program = new cl::Program(*m_context, {m_device}, bins,
                                                NULL, &err),
               m_err_code, c_clinvalidbin, c_clinvalidvalue);
  if (error_code()) {
  ...
  if ((m_cdflow == BOTH) || (m_cdflow == COMP_ONLY)) {
    for (uint8_t i = 0; i < C_COMPUTE_UNIT * OVERLAP_BUF_COUNT; i++) {
      OCL_CHECK(err, m_q[i] = new cl::CommandQueue(*m_context, m_device,
                                                   m_isProfile, &err));
    }
    ...
    for (uint32_t i = 0; i < C_COMPUTE_UNIT; i++) {
      ...
      std::string krnl_name_full =
          comp_krnl_name + ":{" + comp_krnl_name + "_" + cu_id + "}";
      OCL_CHECK(err, compress_kernel[i] = new cl::Kernel(
                         *m_program, krnl_name_full.c_str(), &err));
    }
    ...
    for (uint32_t i = 0; i < H_COMPUTE_UNIT; i++) {
      cu_id = std::to_string(i + 1);
      std::string krnl_name_full =
          huffman_krnl_name + ":{" + huffman_krnl_name + "_" + cu_id + "}";
      OCL_CHECK(err, huffman_kernel[i] = new cl::Kernel(
                         *m_program, krnl_name_full.c_str(), &err));
    }
#endif
  }
  ...
  if ((m_cdflow == BOTH) || (m_cdflow == DECOMP_ONLY)) {
    for (uint8_t i = 0; i < D_COMPUTE_UNIT; i++) {
      OCL_CHECK(err, m_q_dec[i] = new cl::CommandQueue(*m_context, m_device,
                                                       m_isProfile, &err));
    }

    for (uint8_t i = 0; i < D_COMPUTE_UNIT; i++) {
      OCL_CHECK(err, m_q_rd[i] = new cl::CommandQueue(*m_context, m_device,
                                                      m_isProfile, &err));
      OCL_CHECK(err, m_q_rdd[i] = new cl::CommandQueue(*m_context, m_device,
                                                       m_isProfile, &err));
      OCL_CHECK(err, m_q_wr[i] = new cl::CommandQueue(*m_context, m_device,
                                                      m_isProfile, &err));
      OCL_CHECK(err, m_q_wrd[i] = new cl::CommandQueue(*m_context, m_device,
                                                       m_isProfile, &err));
    }
    ...
    for (uint32_t i = 0; i < D_COMPUTE_UNIT; i++) {
      cu_id = std::to_string(i + 1);
      std::string data_writer_kname = data_writer_kernel_name + ":{" +
                                      data_writer_kernel_name + "_" + cu_id +
                                      "}";
      std::string data_reader_kname = data_reader_kernel_name + ":{" +
                                      data_reader_kernel_name + "_" + cu_id +
                                      "}";
      std::string decompress_kname =
          stream_decompress_kernel_name[kidx] + ":{" +
          stream_decompress_kernel_name[kidx] + "_" + cu_id + "}";

      OCL_CHECK(err, data_writer_kernel[i] = new cl::Kernel(
                         *m_program, data_writer_kname.c_str(), &err));
      OCL_CHECK(err, data_reader_kernel[i] = new cl::Kernel(
                         *m_program, data_reader_kname.c_str(), &err));
      OCL_CHECK(err, decompress_kernel[i] = new cl::Kernel(
                         *m_program, decompress_kname.c_str(), &err));
    }
  }
...
// Constructor
xfZlib::xfZlib(const std::string &binaryFileName, uint8_t max_cr,
               uint8_t cd_flow, uint8_t device_id, uint8_t profile,
               uint8_t d_type) {
  ...
  auto duration_devscan = std::chrono::duration<double, std::milli>(
      device_API_end - device_API_start);
  device_API_time_ns_1 = duration_devscan;
  float device_scan = device_API_time_ns_1.count();
  std::cout << "OpenCL Setup = " << std::fixed << std::setprecision(2)
            << device_scan << std::endl;
#endif
  ...
  host_buffer_size =
      ((host_buffer_size - 1) / block_size_in_kb + 1) * block_size_in_kb;
  ...
  if ((cd_flow == BOTH) || (cd_flow == COMP_ONLY)) {
    ...
    for (int i = 0; i < MAX_CCOMP_UNITS; i++) {
      for (int j = 0; j < OVERLAP_BUF_COUNT; j++) {
        ...
        MEM_ALLOC_CHECK(h_buf_in[i][j].resize(HOST_BUFFER_SIZE),
                        HOST_BUFFER_SIZE, "Input Host Buffer");
        MEM_ALLOC_CHECK(h_buf_zlibout[i][j].resize(HOST_BUFFER_SIZE * 2),
                        HOST_BUFFER_SIZE * 2, "Output Host Buffer");
        MEM_ALLOC_CHECK(h_blksize[i][j].resize(MAX_NUMBER_BLOCKS),
                        MAX_NUMBER_BLOCKS, "BlockSize Host Buffer");
        MEM_ALLOC_CHECK(h_compressSize[i][j].resize(MAX_NUMBER_BLOCKS),
                        MAX_NUMBER_BLOCKS, "CompressSize Host Buffer");
      }
    }

#if (VERBOSE_LEVEL >= 2)
    auto cons_API_end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration<double, std::milli>(cons_API_end -
                                                              cons_API_start);
    cons_API_time_ns_1 = duration;
    float cons_time = cons_API_time_ns_1.count();
    std::cout << "Compress Host Buffer Allocation Time = " << std::fixed
              << std::setprecision(2) << cons_time << std::endl;
#endif
    cl_int err;
    for (int i = 0; i < MAX_CCOMP_UNITS; i++) {
      for (int j = 0; j < OVERLAP_BUF_COUNT; j++) {
        // Device Buffer Allocation
        OCL_CHECK(err, buffer_input[i][j] = new cl::Buffer(
                           *m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                           host_buffer_size, h_buf_in[i][j].data(), &err));

        OCL_CHECK(err, buffer_lz77_output[i][j] =
                           new cl::Buffer(*m_context, CL_MEM_READ_WRITE |
                                                          CL_MEM_HOST_NO_ACCESS,
                                          host_buffer_size * 4, NULL, &err));

        OCL_CHECK(err, buffer_compress_size[i][j] = new cl::Buffer(
                           *m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                           temp_nblocks * sizeof(uint32_t),
                           h_compressSize[i][j].data(), &err));
        ...
        OCL_CHECK(err, buffer_inblk_size[i][j] = new cl::Buffer(
                           *m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                           temp_nblocks * sizeof(uint32_t),
                           h_blksize[i][j].data(), &err));

        OCL_CHECK(err,
                  buffer_dyn_ltree_freq[i][j] = new cl::Buffer(
                      *m_context, CL_MEM_READ_WRITE | CL_MEM_HOST_NO_ACCESS,
                      PARALLEL_ENGINES * sizeof(uint32_t) * c_ltree_size, NULL,
                      &err));

        OCL_CHECK(err,
                  buffer_dyn_dtree_freq[i][j] = new cl::Buffer(
                      *m_context, CL_MEM_READ_WRITE | CL_MEM_HOST_NO_ACCESS,
  ...
  if ((cd_flow == BOTH) || (cd_flow == DECOMP_ONLY)) {
    ...
    for (int j = 0; j < DOUT_BUFFERCOUNT; ++j) {
      MEM_ALLOC_CHECK(h_dbufstream_zlibout[j].resize(OUTPUT_BUFFER_SIZE),
                      OUTPUT_BUFFER_SIZE, "Output Host Buffer");
      MEM_ALLOC_CHECK(h_dcompressSize_stream[j].resize(sizeof(uint32_t)),
                      sizeof(uint32_t), "DecompressSize Host Buffer");
    }
    MEM_ALLOC_CHECK(h_dcompressStatus.resize(sizeof(uint32_t)),
                    sizeof(uint32_t), "DecompressStatus Host Buffer");
    ...
    for (int i = 0; i < DIN_BUFFERCOUNT; i++) {
      OCL_CHECK(err, buffer_dec_input[i] = new cl::Buffer(
                         *m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
    ...
    cons_API_time_ns_1 = duration;
    float cons_time = cons_API_time_ns_1.count();
    std::cout << "DeCompress Host Buffer Allocation Time = " << std::fixed
              << std::setprecision(2) << cons_time << std::endl;
#endif
  }
...
int xfZlib::error_code(void) { return m_err_code; }
...
uint64_t xfZlib::compress_buffer(uint8_t *in, uint8_t *out,
                                 uint64_t input_size) {
  uint32_t host_buffer_size = HOST_BUFFER_SIZE;
  out[0] = 120;
  out[1] = 1;
...
uint32_t xfZlib::decompress_file(std::string &inFile_name,
                                 std::string &outFile_name, uint64_t input_size,
                                 int cu) {
  ...
  auto duration = std::chrono::duration<double, std::nano>(
      decompress_API_end - decompress_API_start);
  ...
  float throughput_in_mbps_1 =
      (float)debytes * 1000 / decompress_API_time_ns_1.count();
  std::cout << std::fixed << std::setprecision(3) << throughput_in_mbps_1;
...
void xfZlib::_enqueue_reads(uint32_t bufSize, uint8_t *out,
                            uint32_t *decompSize, int cu,
                            uint32_t max_outbuf_size) {
  ...
  OCL_CHECK(err, buffer_status = new cl::Buffer(
                     *m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
  ...
  do {
    ...
    if (((kernelReadWait[cbf_idx]).size() > 0) || (done)) {
      (hostReadEvent[cbf_idx])
          .wait(); // wait for previous data migration to complete
      if (!done) {
    ...
    if (!done) {
      ...
      OCL_CHECK(err, err = data_reader_kernel[cu]->setArg(
                         0, *(buffer_dec_zlib_output[cbf_idx])));
      OCL_CHECK(err, err = data_reader_kernel[cu]->setArg(
                         1, *(buffer_size[cbf_idx])));

      // enqueue reader kernel
      OCL_CHECK(err,
                err = m_q_rd[cu]->enqueueTask(*data_reader_kernel[cu], NULL,
                                              &(kernelReadEvent[cbf_idx])));
      kernelReadWait[cbf_idx].push_back(
          kernelReadEvent[cbf_idx]); // event to wait for

      OCL_CHECK(err, err = m_q_rdd[cu]->enqueueMigrateMemObjects(
                         {*(buffer_size[cbf_idx]),
                          *(buffer_dec_zlib_output[cbf_idx])},
...
void xfZlib::_enqueue_writes(uint32_t bufSize, uint8_t *in, uint32_t inputSize,
                             int cu) {
  ...
  uint8_t cbf_idx =
      0; // index indicating current buffer(0-4) being used in loop
  ...
  for (keq_idx = 0; keq_idx < bufferCount; ++keq_idx) {
    ...
    OCL_CHECK(err, err = data_writer_kernel[cu]->setArg(
                       0, *(buffer_dec_input[cbf_idx])));
    OCL_CHECK(err, err = data_writer_kernel[cu]->setArg(1, cBufSize));

    // enqueue data migration to kernel
    OCL_CHECK(err, err = m_q_wr[cu]->enqueueMigrateMemObjects(
                       {*(buffer_dec_input[cbf_idx])}, 0, NULL, NULL));
    // enqueue the writer kernel dependent on corresponding bufffer migration
    OCL_CHECK(err, err = m_q_wr[cu]->enqueueTask(*data_writer_kernel[cu], NULL,
                                                 &(kernelWriteEvent[cbf_idx])));
  }
  // wait for enqueued writer kernels to finish
  OCL_CHECK(err, err = m_q_wr[cu]->finish());
}

uint32_t xfZlib::decompress(uint8_t *in, uint8_t *out, uint32_t input_size,
                            int cu) {
  cl_int err;
#if (VERBOSE_LEVEL >= 1)
  std::cout << "CU" << cu << " ";
  ...
  bool ochck =
      std::find(oscodes.cbegin(), oscodes.cend(), oscode_in) == oscodes.cend();
  if (ochck) {
    std::cerr << "\n";
  ...
  if (in[hidx++] == cmf) {
    uint8_t flg = in[hidx];
    bool hchck = std::find(zlib_flags.cbegin(), zlib_flags.cend(), flg) ==
                 zlib_flags.cend();
    if (hchck) {
      std::cerr << "\n";
  ...
  for (int i = 0; i < c_bufcnt; i++) {
    ZOCL_CHECK(err,
               err = data_writer_kernel[cu]->setArg(0, *(buffer_dec_input[i])),
               m_err_code, c_clOutOfResource);
    if (error_code())
      return 0;
  }
  ...
  for (int i = 0; i < c_outBufCnt; i++) {
    ZOCL_CHECK(err, err = data_reader_kernel[cu]->setArg(
                        0, *(buffer_dec_zlib_output[i])),
               m_err_code, c_clOutOfResource);
    if (error_code())
      return 0;
  }
  ...
  if (input_size < inBufferSize)
    inBufferSize = input_size;
  if ((max_outbuf_size < outBufferSize) && (max_outbuf_size != 0))
    outBufferSize = max_outbuf_size;
  ...
  std::thread decompWriter(&xfZlib::_enqueue_writes, this, inBufferSize, in,
                           input_size, cu);
  std::thread decompReader(&xfZlib::_enqueue_reads, this, outBufferSize, out,
                           &decmpSizeIdx, cu, max_outbuf_size);
...
uint64_t xfZlib::compress(uint8_t *in, uint8_t *out, uint64_t input_size,
...
overlap:
  for (uint32_t brick = 0, itr = 0; brick < total_chunks;
    ...
    if (itr >= OVERLAP_BUF_COUNT) {
      ...
      for (uint32_t bIdx = 0; bIdx < blocksPerChunk[brick_flag_idx]; bIdx++) {
        uint32_t compressed_size = h_compressSize[0][buffIdx].data()[bIdx];
        std::memcpy(
            &out[outIdx],
            &h_buf_zlibout[0][buffIdx].data()[bIdx * block_size_in_bytes],
            compressed_size);
    ...
    std::memcpy(h_buf_in[0][buffIdx].data(), &in[brick * host_buffer_size],
                sizeOfChunk[brick]);

    // Set kernel arguments
    int narg = 0;

    compress_kernel[0]->setArg(narg++, *buffer_input[0][buffIdx]);
    compress_kernel[0]->setArg(narg++, *buffer_zlib_output[0][buffIdx]);
    compress_kernel[0]->setArg(narg++, *buffer_compress_size[0][buffIdx]);
    compress_kernel[0]->setArg(narg++, sizeOfChunk[brick]);

    // Migrate memory - Map host to device buffers
    OCL_CHECK(err,
              err = m_q[buffIdx]->enqueueMigrateMemObjects(
                  {*(buffer_input[0][buffIdx])}, 0 /* 0 means from host*/));

    // Compress Fire Kernel invocation
    OCL_CHECK(err, err = m_q[buffIdx]->enqueueTask(*compress_kernel[0]));

    // Migrate memory - Map device to host buffers
    m_q[buffIdx]->enqueueMigrateMemObjects(
        {*(buffer_zlib_output[0][buffIdx]),
         *(buffer_compress_size[0][buffIdx])},
  ...
  for (uint8_t i = 0; i < OVERLAP_BUF_COUNT; i++) {
  ...
  for (uint32_t ovr_itr = 0, brick = stride - overlap_buf_count;
       ovr_itr < leftover; ovr_itr++, brick++) {
...
overlap:
  for (uint32_t brick = 0, itr = 0; brick < total_chunks;
    ...
    for (uint32_t cu = 0; cu < lcl_cu; cu++) {
      ...
      if (itr >= 2) {
        ...
        uint32_t index = 0;
        uint32_t brick_flag_idx =
            brick - (C_COMPUTE_UNIT * overlap_buf_count - cu);

        //////printme("blocksPerChunk %d \n", blocksPerChunk[brick]);
        // Copy the data from various blocks in concatinated manner
        for (uint32_t bIdx = 0; bIdx < blocksPerChunk[brick_flag_idx];
             bIdx++, index += block_size_in_bytes) {
      ...
      uint32_t idxblk = 0;
      for (uint32_t i = 0; i < sizeOfChunk[brick + cu];
           i += block_size_in_bytes) {
        uint32_t block_size = block_size_in_bytes;
      ...
      std::memcpy(h_buf_in[cu][flag].data(),
                  &in[(brick + cu) * host_buffer_size],
                  sizeOfChunk[brick + cu]);

      // Set kernel arguments
      int narg = 0;

      ZOCL_CHECK(err, err = compress_kernel[cu]->setArg(
                          narg++, *(buffer_input[cu][flag])),
                 m_err_code, c_clOutOfResource);
      if (error_code())
        return 0;

      ZOCL_CHECK(err, err = compress_kernel[cu]->setArg(
                          narg++, *(buffer_lz77_output[cu][flag])),
                 m_err_code, c_clOutOfResource);
      if (error_code())
        return 0;

      ZOCL_CHECK(err, err = compress_kernel[cu]->setArg(
                          narg++, *(buffer_compress_size[cu][flag])),
                 m_err_code, c_clOutOfResource);
      if (error_code())
        return 0;

      ZOCL_CHECK(err, err = compress_kernel[cu]->setArg(
                          narg++, *(buffer_inblk_size[cu][flag])),
                 m_err_code, c_clOutOfResource);
      if (error_code())
        return 0;

      ZOCL_CHECK(err, err = compress_kernel[cu]->setArg(
                          narg++, *(buffer_dyn_ltree_freq[cu][flag])),
                 m_err_code, c_clOutOfResource);
      if (error_code())
        return 0;

      ZOCL_CHECK(err, err = compress_kernel[cu]->setArg(
                          narg++, *(buffer_dyn_dtree_freq[cu][flag])),
                 m_err_code, c_clOutOfResource);
      if (error_code())
        return 0;

      OCL_CHECK(err,
                err = compress_kernel[cu]->setArg(narg++, block_size_in_kb));

      OCL_CHECK(err, err = compress_kernel[cu]->setArg(
                         narg++, sizeOfChunk[brick + cu]));

      narg = 0;
      ZOCL_CHECK(err, err = huffman_kernel[cu]->setArg(
                          narg++, *(buffer_lz77_output[cu][flag])),
                 m_err_code, c_clOutOfResource);
      if (error_code())
        return 0;
      ...
      ZOCL_CHECK(err, err = huffman_kernel[cu]->setArg(
                          narg++, *(buffer_zlib_output[cu][flag])),
                 m_err_code, c_clOutOfResource);
      if (error_code())
        return 0;
      ...
      OCL_CHECK(err,
                err = huffman_kernel[cu]->setArg(narg++, block_size_in_kb));
      OCL_CHECK(err, err = huffman_kernel[cu]->setArg(narg++,
                                                      sizeOfChunk[brick + cu]));

      // Migrate memory - Map host to device buffers
      OCL_CHECK(err,
                err = m_q[queue_idx + cu]->enqueueMigrateMemObjects(
                    {*(buffer_input[cu][flag]), *(buffer_inblk_size[cu][flag])},
                    0 /* 0 means from host*/));

      // kernel write events update
      // LZ77 Compress Fire Kernel invocation
      OCL_CHECK(err,
                err = m_q[queue_idx + cu]->enqueueTask(*compress_kernel[cu]));

      // Huffman Fire Kernel invocation
      OCL_CHECK(err,
                err = m_q[queue_idx + cu]->enqueueTask(*huffman_kernel[cu]));
  ...
  // Handle leftover bricks
  for (uint32_t ovr_itr = 0,
                brick = stride - overlap_buf_count * C_COMPUTE_UNIT;
       ovr_itr < leftover; ovr_itr += C_COMPUTE_UNIT, brick += C_COMPUTE_UNIT) {
    lcl_cu = C_COMPUTE_UNIT;
    ...
    for (uint32_t j = 0; j < lcl_cu; j++) {
      ...
      for (uint32_t bIdx = 0; bIdx < blocksPerChunk[brick_flag_idx];
        ...
        uint32_t compressed_size = (h_compressSize[cu][flag].data())[bIdx];
        m_q[queue_idx + cu]->enqueueReadBuffer(
            *(buffer_zlib_output[cu][flag]), CL_TRUE, index,
            compressed_size * sizeof(uint8_t), &out[outIdx]);
        outIdx += compressed_size;
      }
