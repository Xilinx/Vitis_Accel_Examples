...
void xil_compress_decompress_list(const std::string &file_list,
                                  ...
                                  const std::string &ext2, int cu,
                                  const std::string &single_bin, uint8_t max_cr,
  ...
  if (mode != ONLY_DECOMPRESS) {
    ...
    while (std::getline(infilelist, line)) {
      ...
      uint64_t enbytes =
          xlz.compress_file(compress_in, compress_out, input_size);

      std::cout << "\t\t" << (double)input_size / enbytes << "\t\t"
                << std::fixed << std::setprecision(3)
                << (double)input_size / 1000000 << "\t\t\t" << compress_in
                << std::endl;
    }
  }
  ...
  if (mode != ONLY_COMPRESS) {
    ...
    while (std::getline(infilelist_dec, line_dec)) {
      ...
      std::cout << std::fixed << std::setprecision(3) << "\t\t"
                << (double)input_size / 1000000 << "\t\t" << decompress_in
                << std::endl;
    } // While loop ends
  }
}

void xil_batch_verify(const std::string &file_list, int cu, enum list_mode mode,
                      const std::string &single_bin, uint8_t device_id,
                      uint8_t max_cr, const std::string &ext1,
                      const std::string &ext2, const std::string &ext3) {
  // Xilinx ZLIB Compression
  xil_compress_decompress_list(file_list, ext1, ext2, cu, single_bin, max_cr,
                               mode, device_id);
  ...
  std::cout << "---------------------------------------------------------------"
               "-------------------------"
            << std::endl;
  std::cout << "                       Validate: Xilinx GZip Compress vs "
...
void xil_decompress_top(const std::string &decompress_mod, int cu,
                        const std::string &single_bin, uint8_t device_id,
                        uint8_t max_cr) {
  // Xilinx ZLIB object
...
void xil_compress_top(const std::string &compress_mod,
  ...
  // Call ZLIB compression
  uint64_t enbytes =
      xlz.compress_file(lz_compress_in, lz_compress_out, input_size);
...
void xil_validate(const std::string &file_list, const std::string &ext) {
  ...
  while (std::getline(infilelist_val, line_val)) {
    ...
    if (ret == 0) {
      std::cout << (ret ? "FAILED\t" : "PASSED\t") << "\t" << line_in
                << std::endl;
    } else {
...
void xilCompressDecompressTop(const std::string &compress_decompress_mod,
  ...
  if (ret == 0) {
    std::cout << (ret ? "FAILED\t" : "PASSED\t") << "\t" << inputFile
              << std::endl;
  } else {
...
int main(int argc, char *argv[]) {
  ...
  if (!compress_decompress_mod.empty())
    xilCompressDecompressTop(compress_decompress_mod, single_bin, device_id,
                             max_cr_val);
  ...
  if (!filelist.empty()) {
    ...
    xil_batch_verify(filelist, cu_run, lMode, single_bin, device_id, max_cr_val,
                     ".xe2xd.gz", ".xe2xd.gz", ".xe2xd.gz.orig");
  } else if (!compress_mod.empty()) {
    // "-c" - Compress Mode
    xil_compress_top(compress_mod, single_bin, device_id, max_cr_val);
  } else if (!decompress_mod.empty())
    // "-d" - DeCompress Mode
    xil_decompress_top(decompress_mod, cu_run, single_bin, device_id,
                       max_cr_val);
}
