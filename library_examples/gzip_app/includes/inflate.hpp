...
void kStreamReadZlibDecomp(
    hls::stream<ap_axiu<STREAM_WIDTH, 0, 0, 0>> &inKStream,
    hls::stream<ap_uint<STREAM_WIDTH>> &readStream, uint32_t input_size) {
  /**
   * @brief kStreamReadZlibDecomp Read 16-bit wide data from internal streams
   * output by compression modules
...
template <int STREAM_WIDTH>
void kStreamWriteZlibDecomp(
    hls::stream<ap_axiu<STREAM_WIDTH, 0, 0, 0>> &outKStream,
    hls::stream<ap_axiu<64, 0, 0, 0>> &sizestreamd,
    hls::stream<ap_uint<STREAM_WIDTH>> &outDataStream,
    hls::stream<bool> &byteEos, hls::stream<uint64_t> &dataSize) {
  /**
   * @brief kStreamWriteZlibDecomp Read 16-bit wide data from internal streams
   * output by compression modules
   *                                and write to output axi stream.
   *
   * @param outKStream    output kernel stream
   * @param sizestreamd   stream to indicate readable bytes in 512-bit wide
   * stream
...
#pragma HLS dataflow
  ...
  xf::compression::huffmanDecoderStream<c_decoderType, c_byteGenLoopII>(
      inStream, bitunpackstream, bitendofstream, inSize, sizeTrans);

  xf::compression::lzDecompressZlibEosStream<HISTORY_SIZE>(
      bitunpackstream, bitendofstream, outStream, outStreamEoS, outStreamSize,
      sizeTrans);
}
...
void inflateCore(hls::stream<ap_uint<16>> &inStream,
                 hls::stream<ap_uint<8>> &outStream,
                 hls::stream<bool> &outStreamEoS,
                 hls::stream<uint64_t> &outStreamSize, uint32_t input_size) {
  const int c_byteGenLoopII = 1;
  const eHuffmanType c_decoderType = (eHuffmanType)DECODER;
...
#pragma HLS dataflow
  ...
  xf::compression::huffmanDecoder<c_decoderType, c_byteGenLoopII>(
      inStream, bitunpackstream, bitendofstream, input_size);

  xf::compression::lzDecompressZlibEos<HISTORY_SIZE>(
      bitunpackstream, bitendofstream, outStream, outStreamEoS, outStreamSize);
}

template <int DECODER, int PARALLEL_BYTES, int HUFF_LOOP_II = 1,
...
#pragma HLS dataflow
  ...
  xf::compression::huffmanDecoder<c_decoderType, c_byteGenLoopII>(
      inStream, bitunpackstream, bitendofstream, inputSize);

  xf::compression::details::lzProcessingUnit<ap_uint<9>>(
      bitunpackstream, bitendofstream, litLenStream, interLenStream,
      matchLenStream, offsetStream, lzProcOutStream);

  xf::compression::details::lzLiteralUpsizer<PARALLEL_BYTES>(
      lzProcOutStream, interLenStream, litStream);

  xf::compression::lzMultiByteDecompress<PARALLEL_BYTES, HISTORY_SIZE,
                                         ap_uint<9>>(
      litLenStream, litStream, offsetStream, matchLenStream, outStream,
      outStreamEoS, outStreamSize);
}
...
#pragma HLS dataflow
  ...
  details::kStreamReadZlibDecomp<STREAM_WIDTH>(inaxistream, inhlsstream,
                                               input_size);
  details::streamDownsizer<uint32_t, STREAM_WIDTH, 16>(
      inhlsstream, outdownstream, input_size);

  details::inflateCore<DECODER, HISTORY_SIZE, LOW_OFFSET>(
      outdownstream, uncompoutstream, byte_eos, outsize_val, input_size);

  details::upsizerEos<8, STREAM_WIDTH>(uncompoutstream, byte_eos, outhlsstream,
                                       outhlsstream_eos);
  details::kStreamWriteZlibDecomp<STREAM_WIDTH>(
      outaxistream, sizestreamd, outhlsstream, outhlsstream_eos, outsize_val);
}

template <int DECODER, int PARALLEL_BYTES, int HUFF_LOOP_II = 1,
          int HISTORY_SIZE = (32 * 1024)>
void inflateMultiByte(
    hls::stream<ap_axiu<16, 0, 0, 0>> &inaxistream,
    hls::stream<ap_axiu<PARALLEL_BYTES * 8, 0, 0, 0>> &outaxistream,
    hls::stream<ap_axiu<64, 0, 0, 0>> &sizestreamd, uint32_t inputSize) {
  // HUFF_LOOP_II=1 gives better latency but FMax may drop
  // HUFF_LOOP_II=2 gives High FMax but latency will increase
  const int c_byteGenLoopII = HUFF_LOOP_II;
  const int c_parallelBit = PARALLEL_BYTES * 8;
...
#pragma HLS dataflow
  ...
  details::inflateMultiByteCore<DECODER, PARALLEL_BYTES, c_byteGenLoopII,
                                HISTORY_SIZE>(inStream, outStream, outStreamEos,
                                              outSizeStream, inputSize);

  details::kStreamWriteZlibDecomp<c_parallelBit>(
      outaxistream, sizestreamd, outStream, outStreamEos, outSizeStream);
}

template <int DECODER, int PARALLEL_BYTES, int IN_GMEM_DATAWIDTH = 512,
          int OUT_GMEM_DATAWIDTH = 512, int IN_BURST_SIZE = 16,
...
#pragma HLS DATAFLOW
  ...
  details::inflateMultiByteCore<DECODER, PARALLEL_BYTES>(
      inStream, outStream, outStreamEos, outSizeStream, inputSize);
  details::stream2MM<c_krnlOutWidth>(outStream, outStreamEos, outSizeStream,
                                     out, outSize);
}
