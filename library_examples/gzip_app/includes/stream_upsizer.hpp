...
void passUpsizer(hls::stream<ap_uint<IN_WIDTH>> &inStream,
                 ...
                 hls::stream<bool> &inFileEos, hls::stream<bool> &outFileEos) {
...
#pragma HLS PIPELINE II = 1
      ...
      outBuffer.range((byteIdx + 1) * IN_WIDTH - 1, byteIdx * IN_WIDTH) =
          inValue;
...
void simpleUpsizer(hls::stream<ap_uint<IN_WIDTH>> &inStream,
                   hls::stream<bool> &inStreamEos, hls::stream<bool> &inFileEos,
                   hls::stream<ap_uint<OUT_WIDTH>> &outStream,
...
#pragma HLS array_partition variable = outBuffer_int dim = 1 complete
  ...
  /// size,outSize,c_inSize);
  outBuffer_int[byteIdx] = inStream.read();
stream_upsizer:
  for (bool eos_flag = inStream_eos.read(); eos_flag == false;
       eos_flag = inStream_eos.read()) {
#pragma HLS PIPELINE II = 1
    for (int j = 0; j < c_upsizeFactor; j += c_inSize) {
#pragma HLS unroll
      outBuffer.range((j + 1) * c_byteWidth - 1, j * c_byteWidth) =
          outBuffer_int[j];
    }
    byteIdx += 1;
    ////printme("%s: value=%c, chunk_size = %d and
...
void upsizer_sizestream(hls::stream<ap_uint<IN_WIDTH>> &inStream,
  ...
  for (SIZE_DT size = inStreamSize.read(); size != 0;
