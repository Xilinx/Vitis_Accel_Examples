...
/**
 * @file mm2s.hpp
 * @brief Header for modules used for memory mapped to streaming interface
 * conversion.
 *
...
#define STREAM_UTILS_MM2S_IF_NOT_FULL(bIdx, outStream, is_full, read_idx,      \
                                      write_idx, local_buffer)                 \
...
#pragma HLS UNROLL
  ...
  parallel_ops:
    for (uint32_t vid = 0; vid < NUM_BLOCKS; vid++) {
      ...
      uint32_t pendingBytes = (input_size[vid] > read_size[vid])
                                  ? (input_size[vid] - read_size[vid])
                                  : 0;
      uint32_t blkMod = read_size[vid] % BLOCK_SIZE;
      if (pendingBytes && blkMod == 0) {
        uint32_t sendingBytes = BLOCK_SIZE;
        if (pendingBytes < sendingBytes)
          sendingBytes = pendingBytes;
      ...
      if (pendingBytes && !isFull) {
        ...
        uint32_t burstSize =
            (pendingWords > BURST_SIZE) ? BURST_SIZE : pendingWords;
...
template <int OUT_DATAWIDTH = 8, int NUM_BLOCKS = 8, int BLOCK_SIZE = 32768,
          int IN_DATAWIDTH = 512, int BURST_SIZE = 16>
...
#pragma HLS DATAFLOW
  xf::compression::details::mm2multStreamMM1<NUM_BLOCKS, BLOCK_SIZE,
                                             IN_DATAWIDTH, BURST_SIZE>(
      in, outStreamV, outStreamVSize, outSizeStream, input_idx, _input_size);
downsizer:
  for (uint8_t vid = 0; vid < NUM_BLOCKS; vid++) {
#pragma HLS UNROLL
    xf::compression::details::simpleStreamDownSizer<IN_DATAWIDTH,
                                                    OUT_DATAWIDTH>(
        outStreamV[vid], outStreamVSize[vid], outStream[vid]);
  }
}

template <int OUT_DATAWIDTH = 8, int NUM_BLOCKS = 4, int BLOCK_SIZE = 32768,
          int IN_DATAWIDTH = 512, int BURST_SIZE = 16>
void mm2multStreamBlockGenerator(
    const ap_uint<IN_DATAWIDTH> *in,
    hls::stream<ap_uint<OUT_DATAWIDTH>> outStream[NUM_BLOCKS],
    hls::stream<uint32_t> outSizeStream[NUM_BLOCKS],
    hls::stream<uint32_t> outBaseIdx[NUM_BLOCKS], uint32_t input_size) {
#pragma HLS dataflow
  constexpr int c_byteWidth = 8;
  constexpr int c_wordSize = IN_DATAWIDTH / c_byteWidth;
  constexpr int c_max_block_length = 512 * 1024;
  ...
  assert(max_block_length <= c_max_block_length);
...
#pragma HLS ARRAY_PARTITION variable = input_idx dim = 0 complete
  ...
  for (uint8_t j = 0; j < NUM_BLOCKS; j++) {
    ...
    if (readBlockSize + max_block_length > input_size)
      inBlockSize = input_size - readBlockSize;
  ...
  xf::compression::details::mm2multStream1<
      OUT_DATAWIDTH, NUM_BLOCKS, BLOCK_SIZE, IN_DATAWIDTH, BURST_SIZE>(
      in, input_idx, outStream, outSizeStream, input_block_size);
}

template <int DATAWIDTH, int BURST_SIZE, int NUM_BLOCKS>
void mm2sNb(const ap_uint<DATAWIDTH> *in, const uint32_t _input_idx[NUM_BLOCKS],
            hls::stream<ap_uint<DATAWIDTH>> outStream[NUM_BLOCKS],
            const uint32_t _input_size[NUM_BLOCKS]) {
  /**
...
#pragma HLS UNROLL
  ...
  while (pending) {
    pending = 0;
    for (uint32_t bIdx = 0; bIdx < NUM_BLOCKS; bIdx++) {
      uint32_t pending_bytes = (input_size[bIdx] > read_size[bIdx])
                                   ? (input_size[bIdx] - read_size[bIdx])
                                   : 0;
      if ((pending_bytes) && (read_idx[bIdx] == write_idx[bIdx])) {
        uint32_t pending_words = (pending_bytes - 1) / c_wordSize + 1;
        uint32_t burst_size =
            (pending_words > BURST_SIZE) ? BURST_SIZE : pending_words;
        uint32_t mem_read_byte_idx = read_size[bIdx] + input_idx[bIdx];
        uint32_t mem_read_word_idx =
            (mem_read_byte_idx) ? ((mem_read_byte_idx - 1) / c_wordSize + 1)
                                : 0;
      gmem_rd:
...
void mm2multStream(const ap_uint<IN_DATAWIDTH> *in,
   ...
   * writes to the output stream in 8-bit chunks. Writing to the multiple data
   * streams is
   * non-blocking call which is done using is_full() API
   *
...
#pragma HLS DATAFLOW
  ...
  xf::compression::details::mm2sNb<IN_DATAWIDTH, BURST_SIZE, NUM_BLOCKS>(
      in, input_idx, outStreamBuffer, input_size);

// Calling the Downsizer parallelly for entire array of outStreams
parallel_downsizer:
  for (uint32_t i = 0; i < NUM_BLOCKS; i++) {
#pragma HLS UNROLL
    xf::compression::details::streamDownsizer<uint32_t, IN_DATAWIDTH,
                                              OUT_DATAWIDTH>(
        outStreamBuffer[i], outStream[i], input_size[i]);
  }
}
...
#pragma HLS PIPELINE II = 1
      ...
      ap_uint<OUT_DATAWIDTH> tmpValue =
          inBuffer.range((idx + 1) * OUT_DATAWIDTH - 1, idx * OUT_DATAWIDTH);
      outStream << tmpValue;
    }
  }
}

template <int OUT_DATAWIDTH = 8, int NUM_BLOCKS = 8, int IN_DATAWIDTH = 512,
          int BURST_SIZE = 16>
void mm2multStreamSize(
    const ap_uint<IN_DATAWIDTH> *in, const uint32_t input_idx[NUM_BLOCKS],
    hls::stream<ap_uint<OUT_DATAWIDTH>> outStream[NUM_BLOCKS],
    const uint32_t _input_size[NUM_BLOCKS]) {
  /**
   * @brief This module reads 512-bit data from memory interface and
   * writes to the output streams in 8-bit chunks. Writing to the multiple data
   * streams is
...
#pragma HLS DATAFLOW
  xf::compression::details::mm2multStreamSimple<NUM_BLOCKS, IN_DATAWIDTH,
                                                BURST_SIZE>(
      in, outStreamV, outStreamVSize, input_idx, _input_size);
downsizer:
  for (uint8_t vid = 0; vid < NUM_BLOCKS; vid++) {
#pragma HLS UNROLL
    xf::compression::details::mm2multStreamDownSizer<IN_DATAWIDTH,
                                                     OUT_DATAWIDTH>(
        outStreamV[vid], outStreamVSize[vid], outStream[vid]);
  }
}
...
mm2StreamSimple:
  for (uint32_t idx = 0; idx < input_size; idx += c_burstSize) {
    uint32_t pendingBytes =
        (input_size > read_size) ? (input_size - read_size) : 0;
    uint32_t sizeWrite = 0;
    uint32_t pendingWords = (pendingBytes - 1) / c_wordSize + 1;
    uint32_t burstSize =
        (pendingWords > BURST_SIZE) ? BURST_SIZE : pendingWords;
...
#pragma HLS DATAFLOW
  xf::compression::details::mm2SingleStream<GMEM_DATAWIDTH, BURST_SIZE>(
      in, outStreamV, outStreamVSize, _input_size);
  xf::compression::details::mm2StreamDownSizer<GMEM_DATAWIDTH, OUT_DATAWIDTH>(
      outStreamV, outStreamVSize, outStream);
}
    ...
    for (uint32_t bIdx = 0; bIdx < NUM_BLOCKS; bIdx++) {
      ...
      if ((pending_bytes) && (read_idx[bIdx] == write_idx[bIdx])) {
        ...
        if (mem_read_byte_idx)
          mem_read_word_idx = (mem_read_byte_idx % c_wordSize)
                                  ? (mem_read_byte_idx - 1) / c_wordSize
...
void mm2sSimple(const ap_uint<DATAWIDTH> *in,
  ...
  allignedwidth = ((inSize_gmemwidth - allignedwidth) > 0) ? allignedwidth + 1
                                                           : allignedwidth;
...
template <int DATAWIDTH, int BURST_SIZE>
void mm2s(const uintMemWidth_t *in, uintMemWidth_t *head_prev_blk,
          uintMemWidth_t *orig_input_data,
          hls::stream<ap_uint<DATAWIDTH>> &outStream,
          hls::stream<uint32_t> &outStreamSize, uint32_t *compressd_size,
          uint32_t *in_block_size, uint32_t no_blocks,
          uint32_t block_size_in_kb, uint32_t head_res_size, uint32_t offset) {
  const int c_byte_size = 8;
  const int c_word_size = DATAWIDTH / c_byte_size;
...
#pragma HLS RESOURCE variable = buffer core = RAM_2P_LUTRAM
  ...
  for (int bIdx = 0; bIdx < no_blocks + 1; bIdx++) {
    ...
    // printf("[ %s ]blkCompSize %d origSize %d sizeInWord_512 %d offset %d
    // head_res_size %d\n",
    // __FUNCTION__,
