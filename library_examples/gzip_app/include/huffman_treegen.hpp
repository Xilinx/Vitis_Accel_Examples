...
void filter(hls::stream<Frequency> &inFreq, Symbol *heap, uint16_t *heapLength,
...
void filter(Frequency *inFreq, Symbol *heap, uint16_t *heapLength,
            uint16_t *symLength, uint16_t i_symSize) {
  uint16_t hpLen = 0;
  uint16_t smLen = 0;
filter:
  for (uint16_t n = 0; n < i_symSize; ++n) {
...
#pragma HLS PIPELINE II = 1
      Digit digit = (heap[j].frequency >> shift) & (RADIX - 1);
      current_digit[j] = digit;
      ++digit_histogram[digit];
      prev_sorting[j] = heap[j];
    }
    digit_location[0] = 0;
...
template <int SYMBOL_SIZE, int SYMBOL_BITS>
void createTree(Symbol *heap, int num_symbols, ap_uint<SYMBOL_BITS> *parent,
                ap_uint<SYMBOL_SIZE> &left, ap_uint<SYMBOL_SIZE> &right,
                Frequency *frequency) {
  ap_uint<SYMBOL_BITS> tree_count =
      0; // Number of intermediate node assigned a parent
...
#pragma HLS LOOP_TRIPCOUNT min = 19 avg = 286 max = 286
    ...
    if ((in_count < num_symbols && s.frequency <= intermediate_freq) ||
      ...
      // left[i] = INTERNAL_NODE;           // Set symbol to indicate an
      // internal node
      left |= tmp;
...
template <int SYMBOL_SIZE, int SYMBOL_BITS>
void computeBitLength(ap_uint<SYMBOL_BITS> *parent, ap_uint<SYMBOL_SIZE> &left,
                      ap_uint<SYMBOL_SIZE> &right, int num_symbols,
                      Histogram *length_histogram, Frequency *child_depth) {
  ap_uint<SYMBOL_SIZE> tmp;
  // for case with less number of symbols
  if (num_symbols < 3)
    num_symbols++;
...
void truncateTree(Histogram *length_histogram, uint16_t c_tree_depth,
                  int max_bit_len) {
  int j = max_bit_len;
move_nodes:
...
#pragma HLS LOOP_TRIPCOUNT min = 1 max = 1 avg = 1
      ...
      length_histogram[j + 1] +=
          2; // Two new leaf nodes are attached at level j+1
      length_histogram[i - 1] +=
          1;                    // The leaf node at level i+1 gets attached here
...
void canonizeTree(Symbol *sorted, uint32_t num_symbols,
                  Histogram *length_histogram, ap_uint<4> *symbol_bits,
...
#pragma HLS PIPELINE II = 1
    ...
    if (length < 0)
      break;
    symbol_bits[sorted[k].value] =
        length; // assign symbol k to have length bits
    --count;    // keep assigning i bits until we have counted off n symbols
  }
}

template <int MAX_LEN>
void createCodeword(ap_uint<4> *symbol_bits, Histogram *length_histogram,
                    Codeword *huffCodes, uint16_t cur_symSize,
...
#pragma HLS PIPELINE II = 1
  ...
  make_codeword:
    if (length != 0) {
      ...
      huffCodes[k].codeword = (uint16_t)out_reversed;
      huffCodes[k].bitlength = length;
      first_codeword[length]++;
    } else {
      huffCodes[k].codeword = 0;
...
template <int MAX_LEN>
void createCodeword(ap_uint<4> *symbol_bits, Histogram *length_histogram,
                    hls::stream<Codeword> &huffCodes, uint16_t cur_symSize,
                    uint16_t cur_maxBits) {
  //#pragma HLS inline
  typedef ap_uint<MAX_LEN> LCL_Code_t;
  LCL_Code_t first_codeword[MAX_LEN + 1];
...
void huffConstructTreeStream(hls::stream<Frequency> &inFreq,
  ...
  Histogram length_histogram[SYMBOL_SIZE];
...
#pragma HLS PIPELINE II = 1
  ...
  // create tree
  createTree<SYMBOL_SIZE, SYMBOL_BITS>(heap, heapLength, parent, left, right,
                                       temp_array);

  // get bit-lengths from tree
  computeBitLength<SYMBOL_SIZE, SYMBOL_BITS>(parent, left, right, heapLength,
  ...
  // canonize the tree
  canonizeTree<SYMBOL_BITS>(heap, heapLength, length_histogram, symbol_bits,
                            i_treeDepth);

  // generate huffman codewords
  createCodeword<c_tgnMaxBits>(symbol_bits, length_histogram, outCodes,
                               i_symbolSize, i_maxBits);
}

void huffConstructTree(Frequency *inFreq, Codeword *outCodes,
                       uint16_t *maxCodes, uint8_t metaIdx) {
  //#pragma HLS inline
  // construct huffman tree and generate codes and bit-lengths
  const int hctMeta[3][3] = {{c_litCodeCount, c_litCodeCount, c_maxCodeBits},
                             {c_dstCodeCount, c_dstCodeCount, c_maxCodeBits},
...
#pragma HLS PIPELINE II = 1
  ...
  // create tree
  createTree<c_tgnSymbolSize, c_tgnSymbolBits>(heap, heapLength, parent, left,
                                               right, temp_array);

  // get bit-lengths from tree
  computeBitLength<c_tgnSymbolSize, c_tgnSymbolBits>(
      parent, left, right, heapLength, length_histogram, temp_array);

  // truncate tree for any bigger bit-lengths
  truncateTree(length_histogram, i_treeDepth, i_maxBits);

  // canonize the tree
  canonizeTree<c_tgnSymbolBits>(heap, heapLength, length_histogram, symbol_bits,
                                i_treeDepth);
...
void sendTrees(hls::stream<ap_uint<c_tgnSymbolBits>> &maxLdCodes,
               ...
               hls::stream<Codeword> &Ldcodes, hls::stream<Codeword> &Blcodes,
               hls::stream<bool> &isEOBlock, hls::stream<uint16_t> &codeStream,
  ...
  while (isEOBlock.read() == 0) {
    ...
    uint8_t bitlen_vals[19] = {16, 17, 18, 0, 8,  7, 9,  6, 10, 5,
                               11, 4,  12, 3, 13, 2, 14, 1, 15};
    ap_uint<c_tgnSymbolBits> maxCodesReg[3];

    uint16_t offsets[3] = {0, c_litCodeCount + 1,
                           (c_litCodeCount + c_dstCodeCount + 2)};
...
#pragma HLS PIPELINE II = 1
      if ((uint8_t)(outCodes[offsets[2] + bitlen_vals[bl_mxc]].bitlength) != 0)
        break;
    }
...
#pragma HLS PIPELINE II = 1
    ...
    for (int tree = 0; tree < 2; tree++) {
      ...
      uint8_t nextlen =
          (tree == 0) ? outCodes[0].bitlength
                      : outCodes[offsets[1]].bitlength; // Length of next code
      uint8_t count = 0;
...
void codeWordDistributor(
    hls::stream<Codeword> &inStreamCode, hls::stream<Codeword> &outStreamCode1,
...
template <int SLAVES>
void streamDistributor(hls::stream<bool> &inStream,
                       hls::stream<bool> outStream[SLAVES]) {
  do {
...
void processLiteralDistance(hls::stream<Frequency> &frequencies,
  ...
  do {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      details::huffConstructTreeStream<c_litCodeCount, c_tgnSymbolBits>(
          frequencies, outCodes, maxCodes, i);
    }
  } while (1);
}
...
void processBitLength(hls::stream<Frequency> &frequencies,
  ...
  while (isEOBlock.read() == 0) {
    // read freqStream and generate codes for it
    // construct the huffman tree and generate huffman codes
    details::huffConstructTreeStream<c_blnCodeCount, c_tgnBitlengthBits>(
        frequencies, outCodes, maxCodes, 2);
  }
}
...
// DATAFLOW
#pragma HLS DATAFLOW
  streamDistributor<5>(isEOBlock, eoBlock);
  processLiteralDistance(frequencies, eoBlock[0], ldCodes, ldMaxCodes);
  codeWordDistributor(ldCodes, ldCodes1, ldCodes2, ldMaxCodes, ldMaxCodes1,
                      ldMaxCodes2, eoBlock[1]);
  getFrequencies(ldCodes1, ldFrequency, ldMaxCodes1, eoBlock[2]);
  processBitLength(ldFrequency, eoBlock[3], blCodes, blMaxCodes);
  sendTrees(ldMaxCodes2, blMaxCodes, ldCodes2, blCodes, eoBlock[4], codeStream,
            codeSize);
}

} // end namespace details

void zlibTreegenCore(Frequency *inFreq, Codeword *outCodes,
                     uint16_t *maxCodes) {
  // Core module for zlib huffman treegen
  // one value padding between codes of literals-distance-bitlengths, needed
  // later
  ...
  for (int i = 0; i < 3; ++i) {
    // construct the huffman tree and generate huffman codes
    details::huffConstructTree(&(inFreq[offsets[i]]), &(outCodes[offsets[i]]),
                               &(maxCodes[i]), i);
    // only after codes have been generated for literals and distances
    if (i < 2) {
      // generate frequency data for bitlengths
      details::genBitLenFreq(&(outCodes[offsets[i]]), &(inFreq[offsets[2]]),
                             maxCodes[i]);
    }
  }
  ...
  uint8_t bitlen_vals[32] = {16, 17, 18, 0, 8,  7, 9,  6, 10, 5,
                             11, 4,  12, 3, 13, 2, 14, 1, 15};
  uint16_t bl_mxc;
bltree_blen:
  for (bl_mxc = c_blnCodeCount - 1; bl_mxc >= 3; --bl_mxc) {
#pragma HLS PIPELINE II = 3
    if ((uint8_t)(outCodes[bitlen_vals[bl_mxc] + offsets[2]].bitlength) != 0)
      break;
  }
  maxCodes[2] = bl_mxc;
}

void zlibTreegenInMMOutStream(uint32_t *lit_freq, uint32_t *dist_freq,
                              hls::stream<uint16_t> &codeStream,
                              hls::stream<uint8_t> &codeSize) {
  // Core module for zlib huffman treegen
  // Using single array for Frequency and Codes for literals, distances and
  // bit-lengths
  ...
  // initialize all the memory
  for (int i = 0; i < (c_litCodeCount + c_dstCodeCount + c_blnCodeCount + 2);
       ++i) {
    outCodes[i].codeword = 0;
    outCodes[i].bitlength = 0;
  }
