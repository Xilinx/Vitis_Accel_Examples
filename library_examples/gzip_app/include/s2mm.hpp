...
/**
 * @file s2mm.hpp
 * @brief Header for modules used for streaming to memory mapped interface
 * conversion.
 *
...
#define STREAM_UTILS_S2MM_READ_SIZE(i, instream, end_of_stream)                \
  if (!end_of_stream.range(i, i) && !instream.empty()) {                       \
    ...
    if (tmpValue == 0)                                                         \
      end_of_stream.range(i, i) = 1;                                           \
  }

#define STREAM_UTILS_S2MM_IF_NOT_EMPTY(i, instream, burst_size, input_size,    \
                                       read_size, write_size, write_idx)       \
  burst_size[i] = c_max_burst_size;                                            \
  if (end_of_stream.range(i, i) &&                                             \
      ((input_size[i] - write_size[i]) < burst_size[i])) {                     \
    burst_size[i] = GET_DIFF_IF_BIG(input_size[i], write_size[i]);             \
  }                                                                            \
  if (((read_size[i] - write_size[i]) < burst_size[i]) &&                      \
      (input_size[i] > read_size[i])) {                                        \
...
template <int NUM_BLOCK = 4, int DATAWIDTH = 512, int BURST_SIZE = 16,
...
void multStream2MMNB(hls::stream<ap_uint<DATAWIDTH>> inStream[NUM_BLOCK],
                     ...
                     ap_uint<DATAWIDTH> *out, uint32_t *compressedSize) {
...
#pragma HLS PIPELINE
      if (!inCmpBlkSizeStream[i].empty())
        compressedSize[idx++] = inCmpBlkSizeStream[i].read();
    }
  }
}

template <int IN_DATAWIDTH, int NUM_BLOCK = 4, int GMEM_DATAWIDTH = 512,
          int BURST_SIZE = 16, int BLOCK_SIZE = 32768>
void multStream2MMBlockReceiver(
    hls::stream<ap_uint<IN_DATAWIDTH>> inStream[NUM_BLOCK],
    hls::stream<bool> inStreamEos[NUM_BLOCK],
    hls::stream<bool> inFileEos[NUM_BLOCK],
    hls::stream<uint32_t> totalOutSizeStream[NUM_BLOCK],
    hls::stream<uint32_t> inBaseIdx[NUM_BLOCK], ap_uint<GMEM_DATAWIDTH> *out,
    uint32_t *outSize) {
#pragma HLS DATAFLOW
...
parallel_upsizer:
  for (uint8_t i = 0; i < NUM_BLOCK; i++) {
#pragma HLS UNROLL
    xf::compression::details::simpleUpsizer<IN_DATAWIDTH, GMEM_DATAWIDTH,
                                            BURST_SIZE>(
        inStream[i], inStreamEos[i], inFileEos[i], outStreamV[i],
        outStreamVEos[i], outStreamVSize[i]);
  }

  multStream2MMNB<NUM_BLOCK, GMEM_DATAWIDTH, BURST_SIZE, BLOCK_SIZE>(
      outStreamV, outStreamVEos, outStreamVSize, totalOutSizeStream, inBaseIdx,
      out, outSize);
}
...
stream_upsizer:
  for (bool eos_flag = inStreamEos.read(); eos_flag == false;
...
void singleStream2mm(hls::stream<ap_uint<DATAWIDTH>> &inStream,
                     ...
                     ap_uint<DATAWIDTH> *out, ap_uint<DATAWIDTH> *outSize) {
  /**
  ...
  uint32_t memIdx = 0;
s2mm_simple:
  for (uint16_t inSize = inSizeStream.read(); inSize != 0;
       inSize = inSizeStream.read()) {
...
void stream2MM(hls::stream<ap_uint<IN_DATAWIDTH>> &inStream,
               hls::stream<bool> &inStreamEos,
               hls::stream<uint64_t> &outSizeStream,
               ap_uint<GMEM_DATAWIDTH> *out, ap_uint<GMEM_DATAWIDTH> *outSize) {
  /**
...
#pragma HLS DATAFLOW
  xf::compression::details::stream2mmUpsizer<IN_DATAWIDTH, GMEM_DATAWIDTH,
                                             BURST_SIZE>(
      inStream, inStreamEos, outStreamV, outStreamVSize);
  xf::compression::details::singleStream2mm<GMEM_DATAWIDTH>(
      outStreamV, outStreamVSize, outSizeStream, out, outSize);
}
...
void multStream2mmSize(hls::stream<ap_uint<DATAWIDTH>> inStream[NUM_BLOCK],
                       ...
                       ap_uint<DATAWIDTH> *out, uint32_t outSize[NUM_BLOCK]) {
  /**
...
template <int IN_DATAWIDTH, int NUM_BLOCK = 8, int GMEM_DATAWIDTH = 512,
          int BURST_SIZE = 16>
void multStream2MM(hls::stream<ap_uint<IN_DATAWIDTH>> inStream[NUM_BLOCK],
                   hls::stream<bool> inStreamEos[NUM_BLOCK],
                   hls::stream<uint32_t> totalOutSizeStream[NUM_BLOCK],
                   const uint32_t output_idx[NUM_BLOCK],
                   ap_uint<GMEM_DATAWIDTH> *out, uint32_t outSize[NUM_BLOCK]) {
  /**
...
#pragma HLS UNROLL
  ...
  xf::compression::details::multStream2mmSize<BURST_SIZE, GMEM_DATAWIDTH,
                                              NUM_BLOCK>(
      outStreamV, outStreamVSize, totalOutSizeStream, output_idx, out, outSize);
}

template <class STREAM_SIZE_DT, int IN_DATAWIDTH, int NUM_BLOCK = 8,
          int GMEM_DATAWIDTH = 512, int BURST_SIZE = 16>
void multStream2MMFreq(hls::stream<ap_uint<IN_DATAWIDTH>> inStream[NUM_BLOCK],
   ...
   * @param totalOutSizeStream output stream contains compressed size of each
   * block
...
#pragma HLS PIPELINE II = 1
      STREAM_UTILS_S2MM_IF_NOT_EMPTY(0, inStream_0, burst_size, input_size,
                                     read_size, write_size, write_idx);
#if PARALLEL_BLOCK > 1
      STREAM_UTILS_S2MM_IF_NOT_EMPTY(1, inStream_1, burst_size, input_size,
                                     read_size, write_size, write_idx);
#endif
#if PARALLEL_BLOCK > 2
      STREAM_UTILS_S2MM_IF_NOT_EMPTY(2, inStream_2, burst_size, input_size,
                                     read_size, write_size, write_idx);
      STREAM_UTILS_S2MM_IF_NOT_EMPTY(3, inStream_3, burst_size, input_size,
                                     read_size, write_size, write_idx);
#endif
#if PARALLEL_BLOCK > 4
      STREAM_UTILS_S2MM_IF_NOT_EMPTY(4, inStream_4, burst_size, input_size,
                                     read_size, write_size, write_idx);
      STREAM_UTILS_S2MM_IF_NOT_EMPTY(5, inStream_5, burst_size, input_size,
                                     read_size, write_size, write_idx);
      STREAM_UTILS_S2MM_IF_NOT_EMPTY(6, inStream_6, burst_size, input_size,
                                     read_size, write_size, write_idx);
      STREAM_UTILS_S2MM_IF_NOT_EMPTY(7, inStream_7, burst_size, input_size,
                                     read_size, write_size, write_idx);
#endif
    }
    ...
    for (int i = 0; i < PARALLEL_BLOCK; i++) {
      ...
      if ((read_size[i] > write_size[i]) &&
          (read_size[i] - write_size[i]) >= burst_size[i]) {
        uint32_t base_addr = output_idx[i] + write_size[i];
        uint32_t base_idx = base_addr / c_word_size;
        uint32_t burst_size_in_words =
            (burst_size[i]) ? ((burst_size[i] - 1) / c_word_size + 1) : 0;
...
void s2mmNb(ap_uint<DATAWIDTH> *out, const uint32_t output_idx[NUM_BLOCKS],
...
#pragma HLS UNROLL
        ...
        if (((input_size[pb] - write_size[pb]) < burst_size[pb])) {
          burst_size[pb] = (input_size[pb] > write_size[pb])
                               ? (input_size[pb] - write_size[pb])
                               : 0;
        }
        if (((read_size[pb] - write_size[pb]) < burst_size[pb]) &&
            (input_size[pb] > read_size[pb])) {
    ...
    for (int i = 0; i < NUM_BLOCKS; i++) {
      ...
      if ((read_size[i] > write_size[i]) &&
        ...
        uint32_t burst_size_in_words =
            (burst_size[i]) ? ((burst_size[i] - 1) / c_wordSize + 1) : 0;
...
void s2mmEosSimple(ap_uint<DATAWIDTH> *out,
                   hls::stream<ap_uint<DATAWIDTH>> &inStream,
                   hls::stream<bool> &endOfStream,
                   hls::stream<uint32_t> &outSize, uint32_t *output_size) {
  /**
   * @brief This module reads DATAWIDTH data from stream based on
   * size stream and writes the data to DDR.
   *
...
template <int DATAWIDTH>
void s2mmSimple(ap_uint<DATAWIDTH> *out,
                hls::stream<ap_uint<DATAWIDTH>> &inStream,
...
void s2mmStreamSimple(ap_uint<DATAWIDTH> *out,
  ...
  /**
   * @brief This module reads N-bit data from stream based on
   * end of stream and writes the data to DDR. N is template parameter
   * DATAWIDTH.
   *
   * @tparam DATAWIDTH Width of the input data stream
   *
...
template <int DATAWIDTH>
void s2mm(hls::stream<ap_uint<DATAWIDTH>> &inStream, ap_uint<DATAWIDTH> *out,
