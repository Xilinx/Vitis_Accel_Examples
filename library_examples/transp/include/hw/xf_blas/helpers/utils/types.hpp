...
template <typename t_DataType, unsigned int t_Width, unsigned int t_DataWidth = sizeof(t_DataType) * 8>
class TaggedDualWideType {
   private:
    typedef WideType<t_DataType, t_Width, t_DataWidth> t_WideType;
   ...
   public:
    ...
    static const unsigned int t_TypeWidth = t_WideType::t_TypeWidth * 2 + 2;
    typedef ap_uint<t_TypeWidth> t_TypeInt;

    TaggedDualWideType() {}

    TaggedDualWideType(const TaggedDualWideType& p_dt) {
        m_a = p_dt.m_a;
        m_b = p_dt.m_b;
    ...
    TaggedDualWideType(const t_TypeInt& p_val) {
        ap_uint<t_Width* t_DataWidth> l_a = p_val.range(t_TypeWidth - 1, t_TypeWidth - t_WideType::t_TypeWidth);
        ap_uint<t_Width* t_DataWidth> l_b =
            p_val.range(t_TypeWidth - 1 - t_WideType::t_TypeWidth, t_TypeWidth - 2 * t_WideType::t_TypeWidth);
        m_a = l_a;
        m_b = l_b;
        m_exit = p_val[1];
        m_flush = p_val[0];
    ...
    operator const t_TypeInt() {
        t_TypeInt l_val;
        ap_uint<t_DataWidth *t_Width> l_a = m_a, l_b = m_b;

        l_val.range(t_TypeWidth - 1, t_TypeWidth - t_WideType::t_TypeWidth) = l_a;
        l_val.range(t_TypeWidth - 1 - t_WideType::t_TypeWidth, t_TypeWidth - 2 * t_WideType::t_TypeWidth) = l_b;
        l_val[1] = m_exit;
        l_val[0] = m_flush;
        return l_val;
    }
...
class DualTaggedType {
   public:
    ...
    static const unsigned int t_TypeWidth = t_DataWidth * 2 + 2;
    typedef ap_uint<t_TypeWidth> t_TypeInt;
    ...
    DualTaggedType(const t_TypeInt& p_val) {
        ap_uint<t_DataWidth> l_a, l_b;
        l_a = p_val.range(t_TypeWidth - 1, t_TypeWidth - t_DataWidth);
        l_b = p_val.range(t_TypeWidth - 1 - t_DataWidth, t_TypeWidth - 2 * t_DataWidth);
        m_a = *reinterpret_cast<t_DataType*>(&l_a);
        m_b = *reinterpret_cast<t_DataType*>(&l_b);
        m_flush = p_val[1];
        m_exit = p_val[0];
    }
    ...
    operator const t_TypeInt() {
        t_TypeInt l_val;
        ap_uint<t_DataWidth> l_a, l_b;
        l_a = *reinterpret_cast<ap_uint<t_DataWidth>*>(&m_a);
        l_b = *reinterpret_cast<ap_uint<t_DataWidth>*>(&m_b);

        l_val.range(t_TypeWidth - 1, t_TypeWidth - t_DataWidth) = l_a;
        l_val.range(t_TypeWidth - 1 - t_DataWidth, t_TypeWidth - 2 * t_DataWidth) = l_b;
        l_val[1] = m_flush;
        l_val[0] = m_exit;
        return l_val;
    }
