...
typedef struct {
    ...
    XVadd_vadd_uio_map maps[ MAX_UIO_MAPS ];
} XVadd_vadd_uio_info;

/***************** Variable Definitions **************************************/
static XVadd_vadd_uio_info uio_info;
...
static int uio_info_read_name(XVadd_vadd_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XVadd_vadd_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XVadd_vadd_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XVadd_vadd_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XVadd_vadd_Initialize(XVadd_vadd *InstancePtr, const char* InstanceName) {
	XVadd_vadd_uio_info *InfoPtr = &uio_info;
